...
@Component({
	...
	styleUrls: ['./con-seccion.component.scss'],
})
export class ConSeccionComponent extends CBaseComponent implements OnInit {
			...
			this.service
				...
				.subscribe({
					next: (response: any) => {
						if (response.Result) {
							this.model = response.Data;
							const vIndex = this.models.findIndex((item: any) => item.CORR_SECCION === response.Data.CORR_SECCION);
	...
	rowRemoving(e: any) {
		this.service
			.delete(this.fillParam(e.data.CORR_SECCION))
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
