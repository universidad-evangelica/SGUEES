// Qué hace: vista de mantenimiento de Tipo Contacto.
// Cómo: administra el CRUD del catálogo GEN_TIPO_CONTACTO (sin empresa).
...
@Component({
	selector: 'app-gen-tipo-contacto',
	templateUrl: './gen-tipo-contacto.component.html',
	styleUrls: ['./gen-tipo-contacto.component.scss'],
})
export class GenTipoContactoComponent extends CBaseComponent implements OnInit {
	@ViewChild(DataGridMttoComponent, { static: false }) dataGrid!: DataGridMttoComponent;

	protected override etiquetaRegistro = 'el tipo de contacto';
	protected override requiereEmpresaSesion = false;
	protected override mttoPageSize = 5;
	protected override mttoPageSizes = [5, 10, 25, 50, 100];
	protected override mttoGridKeyExpr = 'CORR_TIPO_CONTACTO';
	protected override mttoCampoEstado = ESTADO_FIELD;
	protected override mttoEstadoDescribeField = 'NOMBRE_TIPO_CONTACTO';
	...
	constructor(
		...
		private service: GenTipoContactoService
	) {
	...
	fillParam(xCORR?: number): any {
		return { CORR_TIPO_CONTACTO: xCORR ?? 0 };
	}

	override fillData(xModel?: GenTipoContacto): GenTipoContacto {
		if (xModel !== undefined) {
			return {
				CORR_TIPO_CONTACTO: xModel.CORR_TIPO_CONTACTO,
				NOMBRE_TIPO_CONTACTO: xModel.NOMBRE_TIPO_CONTACTO,
	...
	private ordenarModelsPorCorr(): void {
		...
		this.models = [...this.models].sort(
			(a, b) => Number(a.CORR_TIPO_CONTACTO) - Number(b.CORR_TIPO_CONTACTO)
		);
	}
	...
	protected override aplicarRegistroEnGrid(data: unknown, isAdd: boolean): void {
		...
		const record = this.fillData(data as GenTipoContacto);
		const key = this.mttoGridKeyExpr as keyof GenTipoContacto;
	...
	override cancelar(): void {
		super.cancelar(
			(item: any) => item.CORR_TIPO_CONTACTO === this.modelUpdate.CORR_TIPO_CONTACTO
		);
	}
		...
		this.rowRemovingMtto(e, {
			deleteFn: () =>
				this.convertirErrorMttoEnWarning(
					this.service.delete(this.fillParam(e.data.CORR_TIPO_CONTACTO))
				),
		});
	}
	...
	override bloquear(): void {
		...
		this.dataForm.instance.getEditor('ACTIVO_TIPO_CONTACTO')?.option('readOnly', true);
	}
	...
	override habilitar(): void {
		...
		setTimeout(() => {
			...
			this.dataForm.instance.getEditor('ACTIVO_TIPO_CONTACTO')?.option('readOnly', estadoSoloLectura);
		});
	}

	override setFocus(): void {
		setTimeout(() => {
			this.dataForm.instance.getEditor('NOMBRE_TIPO_CONTACTO')?.focus();
		});
	}
}
