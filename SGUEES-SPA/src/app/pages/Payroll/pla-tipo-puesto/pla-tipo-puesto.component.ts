...
import {
	...
	PlaTipoPuestoService,
} from './pla-tipo-puesto.service';
...
export class PlaTipoPuestoComponent extends CBaseComponent implements OnInit {
	...
	fillParam(
		...
		return {
			CORR_TIPO_PUESTO: xCORR_TIPO_PUESTO ?? 0,
			BUSQUEDA: busqueda,
			ESTADO_TIPO_PUESTO: estado,
			PAGE: page,
			PAGE_SIZE: pageSize,
			...columnFilters,
		};
	...
	onActivarClick(e: any): void {
		const row = e.row?.data as PlaTipoPuesto;
		if (!row) {
			return;
		}
		this.confirmEstado('Activar registro', `Desea activar el tipo de puesto "${row.NOMBRE_TIPO_PUESTO}"?`, () => this.cambiarEstado(row, true));
	}
	...
	onEliminarClick(e: any): void {
		...
		this.confirmEstado('Eliminar registro', `Desea eliminar "${row.NOMBRE_TIPO_PUESTO}"?`, () => this.eliminarRegistro(row));
	}
	...
	onDesactivarClick(e: any): void {
		...
		this.confirmEstado('Desactivar registro', `Desea desactivar "${row.NOMBRE_TIPO_PUESTO}"?`, () => this.cambiarEstado(row, false));
	}
	...
	private configurarDataSource(): void {
		this.models = new CustomStore({
			...
			load: async (loadOptions: any) => {
				...
				if (!response.Result) {
					throw new Error(response.ErrorMessage || 'No se pudo cargar el tipo de puesto.');
				}
	...
	private cambiarEstado(row: PlaTipoPuesto, activo: boolean): void {
		const request = { ...row, ESTADO_TIPO_PUESTO: activo };
	...
	private eliminarRegistro(row: PlaTipoPuesto): void {
		this.loadingVisible = true;
		this.service.delete(row).pipe(take(1)).subscribe({
			next: (response: any) => {
				if (response.Result) {
					...
					this.notifyFx(response.ErrorMessage || 'No se puede eliminar el tipo de puesto porque tiene registros asociados.', NotifyType.Warning);
				}
				this.loadingVisible = false;
			},
			error: (error: any) => {
