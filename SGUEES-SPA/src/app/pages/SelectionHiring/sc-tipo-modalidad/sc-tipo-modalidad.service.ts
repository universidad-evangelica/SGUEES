...
export class ScTipoModalidadService {
	...
	getAll(param: any): Observable<IResult> {
		let xWhere: IParam[] = [{ Parameter: 'CORR_TIPO_MODALIDAD', Value: param.CORR_TIPO_MODALIDAD }];
	...
	update(model: any): Observable<IResult> {
		let xWhere: IParam[] = [{ Parameter: 'CORR_TIPO_MODALIDAD', Value: model.CORR_TIPO_MODALIDAD }];
	...
	getColumns(): any {
		return [
			{ dataField: 'CORR_TIPO_MODALIDAD', caption: 'Corr.', width: 85 },
	...
	getSummary(): any {
		return {
			totalItems: [{ column: 'CORR_TIPO_MODALIDAD', summaryType: 'count', valueFormat: '#,##0', displayFormat: 'Cant: {0}' }],
		};
	}
