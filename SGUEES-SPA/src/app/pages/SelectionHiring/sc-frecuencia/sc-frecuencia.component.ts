...
import {
	...
	ScFrecuenciaService,
} from './sc-frecuencia.service';

const GRID_FILTER_CONFIG = { estadoField: 'ESTADO_FRECUENCIA' };
...
export class ScFrecuenciaComponent extends CBaseComponent implements OnInit {
	...
	fillParam(
		...
		return {
			CORR_FRECUENCIA: xCORR_FRECUENCIA ?? 0,
			BUSQUEDA: busqueda,
			PAGE: page,
			PAGE_SIZE: pageSize,
	...
	onActivarClick(e: any): void {
		const row = e.row?.data as ScFrecuencia;
		if (!row) {
			return;
		}
		...
		this.confirmEstado(
			'Activar registro',
			`Desea activar la frecuencia "${row.NOMBRE_FRECUENCIA}"?`,
	...
	onEliminarClick(e: any): void {
		...
		this.confirmEstado(
			'Eliminar registro',
			`Desea eliminar "${row.NOMBRE_FRECUENCIA}"?`,
	...
	onDesactivarClick(e: any): void {
		...
		this.confirmEstado(
			'Desactivar registro',
			`Desea desactivar "${row.NOMBRE_FRECUENCIA}"?`,
	...
	private configurarDataSource(): void {
		this.models = new CustomStore({
			...
			load: async (loadOptions: any) => {
				...
				if (!response.Result) {
					throw new Error(response.ErrorMessage || 'No se pudo cargar la frecuencia.');
				}
	...
	private cambiarEstado(row: ScFrecuencia, activo: boolean): void {
		const request = { ...row, ESTADO_FRECUENCIA: activo };
	...
	private eliminarRegistro(row: ScFrecuencia): void {
		this.loadingVisible = true;
		this.service
			.delete(row)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						...
						this.notifyFx(
							response.ErrorMessage || 'No se puede eliminar la frecuencia porque tiene registros asociados en otras tablas.',
							NotifyType.Warning
						);
					}
					this.loadingVisible = false;
