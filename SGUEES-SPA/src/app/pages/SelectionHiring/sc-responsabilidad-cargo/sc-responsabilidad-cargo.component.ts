...
export class ScResponsabilidadCargoComponent extends CBaseComponent implements OnInit {
	...
	fillParam(
		...
		return {
			CORR_RESPONSABILIDAD: xCORR_RESPONSABILIDAD ?? 0,
			BUSQUEDA: busqueda,
			PAGE: page,
			PAGE_SIZE: pageSize,
	...
	onActivarClick(e: any): void {
		...
		this.confirmEstado(
			'Activar registro',
			`Desea activar la responsabilidad de cargo "${row.NOMBRE_RESPONSABILIDAD}"?`,
	...
	onEliminarClick(e: any): void {
		...
		this.confirmEstado(
			'Eliminar registro',
			`Desea eliminar "${row.NOMBRE_RESPONSABILIDAD}"?`,
	...
	onDesactivarClick(e: any): void {
		...
		this.confirmEstado(
			'Desactivar registro',
			`Desea desactivar "${row.NOMBRE_RESPONSABILIDAD}"?`,
	...
	private configurarDataSource(): void {
		this.models = new CustomStore({
			...
			load: async (loadOptions: any) => {
				...
				if (!response.Result) {
					throw new Error(response.ErrorMessage || 'No se pudo cargar la responsabilidad de cargo.');
				}
			...
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						...
						this.notifyFx(
							response.ErrorMessage || 'No se puede eliminar la responsabilidad de cargo porque tiene registros asociados en otras tablas.',
							NotifyType.Warning
						);
					}
					this.loadingVisible = false;
