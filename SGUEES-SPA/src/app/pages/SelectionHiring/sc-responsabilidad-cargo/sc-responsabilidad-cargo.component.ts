...
@Component({ selector: 'app-sc-responsabilidad-cargo', templateUrl: './sc-responsabilidad-cargo.component.html', styleUrls: ['./sc-responsabilidad-cargo.component.scss'] })
export class ScResponsabilidadCargoComponent extends CBaseComponent implements OnInit {
	...
	constructor(public override appInfoService: AppInfoService, public override router: ActivatedRoute, private service: ScResponsabilidadCargoService, private messageService: MessageService, private authService: AuthService) {
	...
	onActivarClick(e: any): void {
		const row = e.row?.data as ScResponsabilidadCargo;
		if (!row) return;
	...
	private cambiarEstado(row: ScResponsabilidadCargo, activo: boolean): void {
	...
	private eliminarRegistro(row: ScResponsabilidadCargo): void {
		this.loadingVisible = true;
