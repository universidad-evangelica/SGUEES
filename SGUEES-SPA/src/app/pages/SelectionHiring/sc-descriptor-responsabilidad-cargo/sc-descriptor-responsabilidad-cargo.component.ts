...
import { ScDescriptorResponsabilidadCargoService } from './sc-descriptor-responsabilidad-cargo.service';

type EstadoFiltro = boolean | null;

@Component({ selector: 'app-sc-descriptor-responsabilidad-cargo', templateUrl: './sc-descriptor-responsabilidad-cargo.component.html', styleUrls: ['./sc-descriptor-responsabilidad-cargo.component.scss'] })
export class ScDescriptorResponsabilidadCargoComponent extends CBaseComponent implements OnInit {
	...
	constructor(public override appInfoService: AppInfoService, public override router: ActivatedRoute, private service: ScDescriptorResponsabilidadCargoService, private messageService: MessageService) {
	...
	fillParam(xCORR_RESPONSABILIDAD?: number, page = 1, pageSize = 5, busqueda = '', estado: EstadoFiltro = null, columnFilters: Record<string, any> = {}): any {
		return { CORR_RESPONSABILIDAD: xCORR_RESPONSABILIDAD ?? 0, BUSQUEDA: busqueda, ESTADO_RESPONSABILIDAD: estado, PAGE: page, PAGE_SIZE: pageSize, ...columnFilters };
	}

	override fillData(xModel?: ScDescriptorResponsabilidadCargo): ScDescriptorResponsabilidadCargo {
		if (xModel !== undefined) {
			return {
	...
	onActivarClick(e: any): void {
		const row = e.row?.data as ScDescriptorResponsabilidadCargo;
		if (!row) return;
		this.confirmEstado('Activar registro', 'Desea activar la responsabilidad de cargo "' + row.NOMBRE_RESPONSABILIDAD + '"?', () => this.cambiarEstado(row, true));
	}

	onEliminarClick(e: any): void {
		const row = e.row?.data as ScDescriptorResponsabilidadCargo;
		if (!row) return;
		this.confirmEstado('Eliminar registro', 'Desea eliminar "' + row.NOMBRE_RESPONSABILIDAD + '"?', () => this.eliminarRegistro(row));
	}

	onDesactivarClick(e: any): void {
		const row = e.row?.data as ScDescriptorResponsabilidadCargo;
		if (!row) return;
		this.confirmEstado('Desactivar registro', 'Desea desactivar la responsabilidad de cargo "' + row.NOMBRE_RESPONSABILIDAD + '"?', () => this.cambiarEstado(row, false));
	}
	...
	override setFocus(): void {
		setTimeout(() => this.dataForm.instance.getEditor('NOMBRE_RESPONSABILIDAD')?.focus());
	}

	private configurarDataSource(): void {
		this.models = new CustomStore({
			key: 'CORR_RESPONSABILIDAD',
			loadMode: 'processed',
			cacheRawData: false,
	...
	private getGridFilters(filter: any): { busqueda: string; estado: EstadoFiltro; columnas: Record<string, any> } {
		...
		const visit = (node: any): void => {
			...
			if (typeof node[0] === 'string' && node.length >= 3) {
				const field = node[0];
				const value = node[2];
				if (field === 'ESTADO_RESPONSABILIDAD') {
	...
	private cambiarEstado(row: ScDescriptorResponsabilidadCargo, activo: boolean): void {
		const request = { ...row, ESTADO_RESPONSABILIDAD: activo };
	...
	private eliminarRegistro(row: ScDescriptorResponsabilidadCargo): void {
		this.loadingVisible = true;
