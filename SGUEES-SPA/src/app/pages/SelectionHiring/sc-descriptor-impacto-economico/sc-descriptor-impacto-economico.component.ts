...
import { ScDescriptorImpactoEconomicoService } from './sc-descriptor-impacto-economico.service';

type EstadoFiltro = boolean | null;

@Component({
	selector: 'app-sc-descriptor-impacto-economico',
	templateUrl: './sc-descriptor-impacto-economico.component.html',
	styleUrls: ['./sc-descriptor-impacto-economico.component.scss'],
})
export class ScDescriptorImpactoEconomicoComponent extends CBaseComponent implements OnInit {
	...
	override fillData(xModel?: ScDescriptorImpactoEconomico): ScDescriptorImpactoEconomico {
		if (xModel !== undefined) {
			return {
	...
	onActivarClick(e: any): void {
		const row = e.row?.data as ScDescriptorImpactoEconomico;
		if (!row) {
			return;
		}
	...
	private cambiarEstado(row: ScDescriptorImpactoEconomico, activo: boolean): void {
	...
	private eliminarRegistro(row: ScDescriptorImpactoEconomico): void {
		this.loadingVisible = true;
