...
@Component({
	selector: 'app-ban-documento',
	templateUrl: './ban-documento.component.html',
	styleUrls: ['./ban-documento.component.scss'],
})
export class BanDocumentoComponent extends CBaseComponent implements OnInit {
			...
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						...
						this.aplicarTipoBeneficiarioDefault();
					}
				},
				error: (error: any) => this.notifyFx(error, NotifyType.Error),
			});
	...
	private aplicarTipoBeneficiarioDefault(forzar = false): void {
		...
		this.applyTipoBeneficiarioSelection(tipoDefault);
	}
	...
	documentoRowKey(row: BanDocumento): string {
		return `${row.ANIO_PERIODO}-${row.MES_PERIODO}-${row.CORR_TIPO_MOVIMIENTO}-${row.CORR_DOCUMENTO}`;
	}

	override fillData(xModel?: BanDocumento): BanDocumento {
		if (xModel !== undefined) {
			return { ...xModel };
		}
		...
		this.guardarMtto({
			...
			onSuccess: (data: unknown, isAdd: boolean) => {
				...
				if (isAdd) {
					...
					const index = this.models.findIndex((item: BanDocumento) => this.documentoRowKey(item) === key);
					if (index >= 0) {
	...
	override cancelar(): void {
		...
		if (this.banderaMtto === UpdateType.Add || this.banderaMtto === UpdateType.Update) {
			this.confirmaCancelar(() => {
				...
				const vIndex = this.models.findIndex((item: BanDocumento) => this.documentoRowKey(item) === key);
				if (vIndex >= 0) {
	...
	onTipoBeneficiarioChanged(value: number): void {
		if (!value) {
		...
		if (tipo) {
			this.applyTipoBeneficiarioSelection(tipo);
		}
	}
	...
	consultarDetalles(): void {
		...
		this.detaService
			...
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						this.detalles = (response.Data || []).map((item: BanDocumentoDeta) => this.enriquecerDetalle(item));
	...
	enriquecerDetalle(item: BanDocumentoDeta): BanDocumentoDeta {
	...
	cuentaContableDisplay(row: BanDocumentoDeta): string {
		return row?.CUENTA_CONTABLE || '';
	}

	codigoCentroDisplay(row: BanDocumentoDeta): string {
	...
	centroCostoNombreDisplay(row: BanDocumentoDeta): string {
		if (!row?.CORR_CENTRO_COSTO) {
			return 'No Definido';
		}
	...
	private precargarCentrosDetalle(detalles: BanDocumentoDeta[]): void {
	...
	private buildDetallePayload(data: any): BanDocumentoDeta {
		...
		this.rowRemovingMtto(e, {
			...
			reload: () => {
				this.models = (this.models || []).filter(
					(item: BanDocumento) => this.documentoRowKey(item) !== removedKey
				);
			},
		});
	}
