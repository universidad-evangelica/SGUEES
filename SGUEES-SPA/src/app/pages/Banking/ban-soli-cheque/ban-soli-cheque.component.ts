...
import { BanSoliChequeService } from './ban-soli-cheque.service';
import { BanSoliChequeDetaService } from './ban-soli-cheque-deta/ban-soli-cheque-deta.service';
...
@Component({
	selector: 'app-ban-soli-cheque',
	templateUrl: './ban-soli-cheque.component.html',
	styleUrls: ['./ban-soli-cheque.component.scss'],
})
export class BanSoliChequeComponent extends CBaseComponent implements OnInit {
	@ViewChild('gridDetalle', { static: false }) gridDetalle!: DxDataGridComponent;

	protected override etiquetaRegistro = 'la solicitud de cheque';
	protected override requiereEmpresaSesion = true;
	protected readonly lookupOpcion = 'BAN_SOLI_CHEQUE';
	btnEnviar = '';
	btnCancelar = '';
	detalles: BanSoliChequeDeta[] = [];
	readOnly = false;
	...
	constructor(
		...
		private detaService: BanSoliChequeDetaService,
		...
		this.selectedLookUpESTADO_DOCUMENTO = this.selectedLookUpESTADO_DOCUMENTO.bind(this);
	}
	...
	getESTADO_DOCUMENTO() {
		this.mESTADO_DOCUMENTO = [
			{ ESTADO_DOCUMENTO: 'DI', NOMBRE_ESTADO_DOCUMENTO: 'Digitado' },
			{ ESTADO_DOCUMENTO: 'SO', NOMBRE_ESTADO_DOCUMENTO: 'Solicitado' },
			{ ESTADO_DOCUMENTO: 'AP', NOMBRE_ESTADO_DOCUMENTO: 'Aplicado' },
			{ ESTADO_DOCUMENTO: 'EM', NOMBRE_ESTADO_DOCUMENTO: 'Emitido' },
		];
	}
	...
	documentoRowKey(row: BanSoliCheque): string {
		return `${row.ANIO_PERIODO}-${row.MES_PERIODO}-${row.CORR_TIPO_MOVIMIENTO}-${row.CORR_DOCUMENTO}`;
	}

	override fillData(xModel?: BanSoliCheque): BanSoliCheque {
		if (xModel !== undefined) {
			return { ...xModel };
		}
	...
	guardar(): void {
		if (
			...
			this.notifyFx(
				'Solo se pueden modificar solicitudes DIGITADAS. Las solicitudes SOLICITADAS, APLICADAS o EMITIDAS no se pueden modificar.',
				NotifyType.Warning
			);
			return;
		}
		...
		this.guardarMtto({
			...
			onSuccess: (data: unknown, isAdd: boolean) => {
				const row = data as BanSoliCheque;
				...
				if (isAdd) {
					...
					const index = this.models.findIndex((item: BanSoliCheque) => this.documentoRowKey(item) === key);
					if (index >= 0) {
	...
	refrescarBotones(): void {
		if (this.banderaMtto !== UpdateType.Browse) {
			this.btnEnviar =
				this.model?.ESTADO_DOCUMENTO === 'DI' && this.permiteEdit ? 'Enviar Solicitud' : '';
			this.btnCancelar =
				this.model?.ESTADO_DOCUMENTO === 'SO' && this.permiteEdit ? 'Cancelar Solicitud' : '';
		} else {
			this.btnEnviar = '';
			this.btnCancelar = '';
		}
	...
	enviarSolicitud(): void {
		if (this.model?.ESTADO_DOCUMENTO !== 'DI') {
			this.notifyFx('Solo se pueden enviar solicitudes DIGITADAS.', NotifyType.Warning);
			return;
		}
		if (!this.hasDocumentoKeys()) {
			this.notifyFx('Guarde la solicitud antes de enviarla.', NotifyType.Warning);
			return;
		}

		const confirma = custom({
			title: 'Confirmación',
			messageHtml: '¿Realmente quiere enviar esta solicitud de cheque?',
			buttons: [
				{
					text: 'Sí',
					onClick: () => {
						...
						this.service
							.enviarSolicitud(this.model)
							.pipe(take(1))
							.subscribe({
								next: (response: any) => {
									this.loadingVisible = false;
									if (response.Result && response.ErrorCode === 0) {
										const row = response.Data as BanSoliCheque;
										this.model = this.fillData(row);
										this.modelUpdate = this.fillData(row);
										this.aplicarReadOnlyFormulario(true);
										this.refrescarBotones();
										this.notifyFx('Solicitud enviada con éxito', NotifyType.Success);
									} else {
	...
	cancelarSolicitud(): void {
		if (this.model?.ESTADO_DOCUMENTO !== 'SO') {
			this.notifyFx('Solo se pueden cancelar solicitudes en estado SOLICITADO.', NotifyType.Warning);
			return;
		}

		const confirma = custom({
			title: 'Confirmación',
			messageHtml: '¿Realmente quiere cancelar esta solicitud de cheque?',
			buttons: [
				{
					text: 'Sí',
					onClick: () => {
							...
							.subscribe({
								next: (response: any) => {
									...
									if (response.Result && response.ErrorCode === 0) {
										...
										this.habilitar();
										this.refrescarBotones();
										this.notifyFx('Solicitud cancelada con éxito', NotifyType.Success);
									} else {
										this.notifyFx(response.ErrorMessage, NotifyType.Error);
									}
								},
	...
	override cancelar(): void {
		...
		if (this.banderaMtto === UpdateType.Add || this.banderaMtto === UpdateType.Update) {
			this.confirmaCancelar(() => {
				...
				const vIndex = this.models.findIndex((item: BanSoliCheque) => this.documentoRowKey(item) === key);
				if (vIndex >= 0) {
			...
			if (this.banderaMtto === UpdateType.Not_Defined) {
				this.restaurarFilaGridConsulta(
					(item: BanSoliCheque) => this.documentoRowKey(item) === this.documentoRowKey(this.modelUpdate)
				);
			}
			this.AsignaStatus(UpdateType.Browse);
			finalizarCancelacion();
	...
	consultarDetalles(): void {
		...
		this.detaService
			...
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						this.detalles = (response.Data || []).map((item: BanSoliChequeDeta) => this.enriquecerDetalle(item));
	...
	enriquecerDetalle(item: BanSoliChequeDeta): BanSoliChequeDeta {
	...
	cuentaContableDisplay(row: BanSoliChequeDeta): string {
		return row?.CUENTA_CONTABLE || '';
	}

	codigoCentroDisplay(row: BanSoliChequeDeta): string {
	...
	centroCostoNombreDisplay(row: BanSoliChequeDeta): string {
		if (!row?.CORR_CENTRO_COSTO) {
			return 'No Definido';
		}
	...
	private precargarCentrosDetalle(detalles: BanSoliChequeDeta[]): void {
	...
	private buildDetallePayload(data: any): BanSoliChequeDeta {
	...
	rowRemoving(e: any): void {
		if (!this.documentoEditablePorEstado(e.data?.ESTADO_DOCUMENTO)) {
			e.cancel = true;
			this.notifyFx(
				'Solo se pueden eliminar solicitudes DIGITADAS. Las solicitudes SOLICITADAS, APLICADAS o EMITIDAS no se pueden eliminar desde aquí.',
				NotifyType.Warning
			);
			return;
		}
		...
		this.rowRemovingMtto(e, {
			...
			reload: () => {
				this.models = (this.models || []).filter(
					(item: BanSoliCheque) => this.documentoRowKey(item) !== removedKey
				);
			},
		});
	}
	...
	selectedLookUpESTADO_DOCUMENTO(vRow: any): any {
		return vRow[0]?.ESTADO_DOCUMENTO;
	}
