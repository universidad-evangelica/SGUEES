...
export class ComSoliCotizacionDetaDocRepository {
	readonly xController = 'COM_SOLI_COTIZACION_DETA_DOC';
  ...
  insertDoc(model: FormData): any {
    return this.objData.Post(model, this.xController, 'PostDoc', environment.UrlCOMPRASAPI);
  }

  getDoc(xWhere: IParam[]): Observable<Blob> {
		return this.objData.GetBlob(this.xController, 'getDoc', xWhere, environment.UrlCOMPRASAPI);
	}
}
