...
import { ComProveedorService } from './com-proveedor.service';
import { IParam } from 'src/app/FxAPI/IParam';
import { ComProveedorDoc } from './com-proveedor-doc/models/com-proveedor-doc';
import { DxFormComponent } from 'devextreme-angular/ui/form';
import { DxDataGridComponent } from 'devextreme-angular';
import { DomSanitizer, SafeUrl } from '@angular/platform-browser';

@Component({
	selector: 'app-com-proveedor',
	templateUrl: './com-proveedor.component.html',
	styleUrls: ['./com-proveedor.component.scss'],
})
export class ComProveedorComponent extends CBaseComponent implements OnInit {
  @ViewChild('fDocumento', { static: false }) dataDocForm!: DxFormComponent;
  @ViewChild('DataCotizaProv', { static: false }) gDataCotizaProv!: DxDataGridComponent;
  ...
  constructor(
		...
		private service: ComProveedorService,
	...
	getCORR_TIPO_DIP() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_TIPO_DIP', 'GetCORR_TIPO_DIP', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	getCORR_ACTIVIDAD_ECONOMICA() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'COM_ACTIVIDAD_ECONOMICA', 'GetCORR_ACTIVIDAD_ECONOMICA', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						this.mCORR_ACTIVIDAD_ECONOMICA = response.Data;
            console.log(this.mCORR_ACTIVIDAD_ECONOMICA)
					}
				},
				error: (error: any) => {
					this.notifyFx(error, NotifyType.Error);
	...
	getCODIGO_PAIS() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_PAIS', 'GetCODIGO_PAIS', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	getCODIGO_DEPTO() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_DEPTO', 'GetCODIGO_DEPTO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
    ...
    let xWhere: IParam[] = [{ Parameter: 'CODIGO_DEPTO', Value: CODIGO_DEPTO }];
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_MUNICIPIO', 'GetCODIGO_MUNICIPIO', xWhere, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	getCORR_FORMA_PAGO() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_FORMA_PAGO', 'GetCORR_FORMA_PAGO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	getCORR_BANCO() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'COM_BANCO', 'GetCORR_BANCO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	getESTADO_PROVEEDOR() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_LISTA', 'GetESTADO_PROVEEDOR', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	getESTADO_PROVEEDOR_WEB() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_LISTA', 'GetESTADO_PROVEEDOR_WEB', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
  ...
  getTIPO_PERSONERIA() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_LISTA', 'GetTIPO_PERSONERIA', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
  ...
  getESTADO_USUARIO() :void {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'GEN_LISTA', 'GetESTADO_USUARIO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
  ...
  getCORR_TIPO_DOCUMENTO() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'COM_TIPO_DOC_FISICO', 'GetCORR_TIPO_DOCUMENTO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
  ...
  getCORR_CONDICION_PAGO() {
		this.appInfoService
			.getLookUp('COM_PROVEEDOR', 'COM_CONDICION_PAGO', 'getCORR_CONDICION_PAGO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	override fillData(xModel?: ComProveedor): ComProveedor {
		if (xModel !== undefined) {
			return {
  ...
  override editarClick(e: any) {
    ...
    this.getCORR_ACTIVIDAD_ECONOMICA();
    this.consultarCOM_PROVEEDOR_USUARIO();
  ...
  consultarCOM_PROVEEDOR_USUARIO() {
		this.service
			.getAllCOM_PROVEEDOR_USUARIO({ CORR_PROVEEDOR: this.model.CORR_PROVEEDOR })
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
						this.mCOM_PROVEEDOR_USUARIO = response.Data;
					}
				},
				error: (error: any) => {
					this.notifyFx(error, NotifyType.Error);
  ...
  editarComProveedorUsuarioClick(e: any): void  {
    e.event.preventDefault();
    e.component.editRow(e.row.rowIndex);
  };

  gDataComProveedorUsuarioInitNewRow(e: any) {
    e.data.CORR_PROVEEDOR = this.model.CORR_PROVEEDOR;
		e.data.LOGIN_SISTEMA = '';
		e.data.NOMBRE_USUARIO = '';
		e.data.ESTADO_USUARIO = 1;
		e.data.CORREO_ELECTRONICO = '';
  }

  gDataComProveedorUsuarioRowInserting(e: any) {
		const isCanceled = new Promise((resolve, reject) => {
			this.loadingVisible = true;

			let insertDeta = (data: any) => {
				this.service
					.insertCOM_PROVEEDOR_USUARIO(data)
					.pipe(take(1))
					.subscribe({
						next: (response: any) => {
							if (response.Result) {
        ...
        this.AsignaStatus(UpdateType.Add);
				if (!this.service.esValido(this.model, this.notifyFx)) {
					this.loadingVisible = false;
					reject('Debe completar la información del Proveedor');
				}
  ...
  gDataComProveedorUsuarioRowUpdated(e: any) {
		const isCanceled = new Promise((resolve, reject) => {
		this.loadingVisible = true;
		this.service
			.updateCOM_PROVEEDOR_USUARIO(e.data)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	gDataComProveedorUsuarioRowRemoving(e: any) {
		const isCanceled = new Promise((resolve, reject) => {
		this.loadingVisible = true;
		this.service
			.deleteCOM_PROVEEDOR_USUARIO(e.data)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	gDataComProveedorUsuarioRowDblClick(e: any) {
		if (e.rowType === 'data' && this.banderaMtto === UpdateType.Update) {
			e.component.editRow(e.rowIndex);
		}
	}

  AgregarUsuario() {
    this.gDataCotizaProv.instance.addRow();
  }

  gDataComProveedorUsuarioFocusedRowChanged(e: any) {
    this.vFocucedRowClave = e.row;
  }
  //#region <COM_PROVEEDOR_DOC>
  gDataProveedorDocRowDblClick(e: any) {
    if (e.rowType === 'data') {
      this.loadingVisible = true;
      const param = {
        CORR_EMPRESA: this.appInfoService.CORR_EMPRESA,
  ...
  gDataProveedorDocRowRemoving(e: any) {
    const isCanceled = new Promise((resolve, reject) => {
    this.loadingVisible = true;
    this.service
  ...
  consultarDocumentos(): void {
    this.service
    .getAllCOM_PROVEEDOR_DOC({ CORR_PROVEEDOR: this.model.CORR_PROVEEDOR })
    .pipe(take(1))
    .subscribe({
      next: (response: any) => {
        if (response.Result) {
  ...
  //#region <Re-Asignación_CLAVE>
  mostrarPopup() {
    if (typeof this.vFocucedRowClave !== 'undefined') {
      this.popupCambiarClaveVisible = true;
    } else {
      this.notifyFx('Debe seleccionar el usuario al que se le reasignara la clave', NotifyType.Error);
    }
  }

  hidePopup() {
    this.popupCambiarClaveVisible = false;
  ...
  reasignarPass(vFocucedRowClave: any) {
    this.cambiarClave.LOGIN_SISTEMA = vFocucedRowClave.data.LOGIN_SISTEMA;
    if (this.cambiarClave.CLAVE_USUARIO_NUEVA !== this.cambiarClave.CLAVE_CONFIRMAR) {
       return this.notifyFx('Las claves no coinciden', NotifyType.Warning);
    }
    this.cambiarClave.CLAVE_USUARIO= this.cambiarClave.CLAVE_USUARIO_NUEVA;
    this.service
    .reasignarClave(this.cambiarClave)
    .pipe(take(1))
    .subscribe({
      next: (response: any) => {
        if (response.Result) {
          this.notifyFx('Clave cambiada con exito!', NotifyType.Success);
          this.popupCambiarClaveVisible = false;
          this.cambiarClave.LOGIN_SISTEMA = '';
          this.cambiarClave.CLAVE_USUARIO_NUEVA = '';
          this.cambiarClave.CLAVE_CONFIRMAR = '';
