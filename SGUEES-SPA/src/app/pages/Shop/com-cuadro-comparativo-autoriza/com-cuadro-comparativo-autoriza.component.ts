...
import { ComCuadroComparativoService } from './com-cuadro-comparativo-autoriza.service';
import { DxDataGridComponent } from 'devextreme-angular/ui/data-grid';
import { ComCuadroComparativoSoliCotizacion } from './models/com-cuadro-comparativo_soli_cotizacion';
import { DomSanitizer, SafeUrl } from '@angular/platform-browser';
import { ComCuadroComparativoComentario } from './com-cuadro-comparativo-comentario/models/com-cuadro-comparativo-comentario';
import { TabConfig } from './com-cuadro-comparativo-comentario/models/tab-config';
import { DxFormComponent } from 'devextreme-angular/ui/form';
import { ComCuadroComparativoDoc } from './com-cuadro-comparativo-doc/models/com-cuadro-comparativo-doc';


@Component({
	selector: 'app-com-cuadro-comparativo-autoriza',
	templateUrl: './com-cuadro-comparativo-autoriza.component.html',
	styleUrls: ['./com-cuadro-comparativo-autoriza.component.scss'],
})

export class ComCuadroComparativoAutorizaComponent extends CBaseComponent implements OnInit {
  ...
  btnAutorizar = '';
  btnDenegar = '';
  mComentario: ComCuadroComparativoComentario = {
  ...
  constructor(
	...
	getESTADO_CUADRO_COMPARATIVO() {
		this.appInfoService
			.getLookUp('COM_CUADRO_COMPARATIVO_AUTORIZA', 'COM_LISTA', 'GetESTADO_CUADRO_COMPARATIVO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
  ...
  getCORR_TIPO_DOCUMENTO() {
		this.appInfoService
			.getLookUp('COM_CUADRO_COMPARATIVO_AUTORIZAR', 'COM_TIPO_DOC_FISICO', 'GetCORR_TIPO_DOCUMENTO', undefined, environment.UrlCOMPRASAPI)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
	...
	//#region <Metodos Mtto>
	fillParam(xANIO_PERIODO?: number, xCORR_CUADRO_COMPARATIVO?: number): any {
  ...
  refrescarBotones()
  {
    if (this.banderaMtto !== UpdateType.Browse)
      {
        this.btnAutorizar = 'Autorizar';
        this.btnDenegar = 'Denegar';
      } else {
        this.btnAutorizar = '';
        this.btnDenegar = '';
      }
  ...
  override editarClick(e: any) {
    ...
    this.vPERMITE_MODIFICAR = e.row.data.PERMITE_MODIFICAR;
	}
  ...
  override getPermiteEditar(e: any): boolean {
		if (this.permiteEdit && (e.row.data.ESTADO_CUADRO_COMPARATIVO === 'DI' || (e.row.data.ESTADO_CUADRO_COMPARATIVO === 'SO' && e.row.data.PERMITE_MODIFICAR === true))) {
			return true;
		}
		return false;
	}
...
onSelectionChanged(data: any, event: any,CORR_PROVEEDOR: number) {
    ...
    if (this.model.ESTADO_CUADRO_COMPARATIVO == 'SO' && this.vPERMITE_MODIFICAR == false)
    {
        this.loadingVisible = false;
        this.notifyFx('No se pueden hacer mas modificaciones a este cuadro comparativo.', NotifyType.Error);
        return;
    }
...
consultarCOM_CUADRO_COMPARATIVO_COMENTARIO() {
  ...
  override cancelar(): void {
	...
	rowRemoving(e: any) {
		this.service
			.delete(this.fillParam(e.data.ANIO_PERIODO,e.data.CORR_CUADRO_COMPARATIVO))
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
  ...
  GuardarModal(): void
  {
    ...
    if(this.vCORR_SOLI_COTIZACION != 0)
    {
      this.modelCuadroComparativo = {
        CORR_EMPRESA: 0,
        ANIO_PERIODO: this.vANIO_PERIODO_SOLI_COTIZACION,
        CORR_CUADRO_COMPARATIVO: 0,
        FECHA_CUADRO_COMPARATIVO: new Date(),
  ...
  Autorizar(): void {
    const confirpreli = custom({
      ...
      messageHtml: '¿Realmente quiere autorizar el cuadro comparativo?',
      buttons: [
        {
          text: 'Si',
          onClick: (e: any) => {
            ...
            {
              this.service
              .Autorizar(this.model)
              .pipe(take(1))
              .subscribe({
                next: (response: any) => {
                  if (response.Result) {
                    this.notifyFx('Autorizado con exito..!', NotifyType.Success);
                    this.consultar();
                    this.AsignaStatus(UpdateType.Browse);
                  } else {
                    this.notifyFx(response.ErrorMessage, NotifyType.Error);
  ...
  Rechazar(): void {
    const confirpreli = custom({
      title: 'Confirmación de Cuadro Comparativo',
      messageHtml: '¿Realmente quiere ANULAR el cuadro comparativo?',
      buttons: [
        {
          text: 'Si',
          onClick: (e: any) => {
            ...
            if(this.model.CORR_CUADRO_COMPARATIVO>0)
            {
              ...
              this.mComentario.CORR_CUADRO_COMPARATIVO = this.model.CORR_CUADRO_COMPARATIVO;
  ...
  guardarComentarioPopup() {
    ...
    this.loadingVisible = true;
    this.service
			.insertCOM_CUADRO_COMPARATIVO_COMENTARIO(this.mComentario)
			.pipe(take(1))
			.subscribe({
				next: (response: any) => {
					if (response.Result) {
    ...
    .Rechazar(this.model)
      ...
      .subscribe({
        next: (response: any) => {
          if (response.Result) {
            this.notifyFx('Rechazado con exito..!', NotifyType.Success);
            this.consultar();
            this.AsignaStatus(UpdateType.Browse);
          } else {
            this.notifyFx(response.ErrorMessage, NotifyType.Error);
