...
import { ComDocumentoDesAplicarRepository } from './com-documento-desaplicar.repository';
...
export class ComDocumentoDesAplicarService {
	constructor(
    private repo: ComDocumentoDesAplicarRepository,
  ...
  getAllDesAplicar(param: any): Observable<IResult> {
		let xWhere: IParam[] = [
  ...
  DesAplicar(model: any): Observable<IResult> {
		let xWhere: IParam[] = [
      { Parameter: 'ANIO_PERIODO', Value: model.ANIO_PERIODO },
      { Parameter: 'MES_PERIODO', Value: model.MES_PERIODO },
      { Parameter: 'CORR_DOCUMENTO', Value: model.CORR_DOCUMENTO },
