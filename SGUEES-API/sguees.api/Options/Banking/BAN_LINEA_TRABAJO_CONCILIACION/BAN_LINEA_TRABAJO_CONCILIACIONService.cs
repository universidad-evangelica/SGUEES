...
namespace sguees.Services
{
	public class BAN_LINEA_TRABAJO_CONCILIACIONService : IBAN_LINEA_TRABAJO_CONCILIACIONService
	{
		...
		public async Task<CResult> GetAllAsync(BAN_LINEA_TRABAJO_CONCILIACIONParam xWhere)
		{
			...
			if (xWhere.AUMENTA_DISMINUYE != 0)
			{
				p.Add(new CParameter() { ParameterName = "AUMENTA_DISMINUYE", Value = xWhere.AUMENTA_DISMINUYE, DbType = System.Data.DbType.Int32 });
			}
		...
		public async Task<CResult> ActivarInactivarAsync(BAN_LINEA_TRABAJO_CONCILIACIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			if (Data.CORR_LINEA <= 0)
			{
				return new CResult
				{
					...
					CodeHelper = 0,
					ErrorCode = -1,
					ErrorMessage = "No se pudo identificar la línea de trabajo a actualizar.",
					ErrorSource = "[BAN_LINEA_TRABAJO_CONCILIACIONService]",
					RowsAffected = 0
				};
			}
