...
namespace scuees.Services
{
	...
	{
		private readonly ICOM_SOLI_COTIZACION_DETA_DOCRepository _repo;
		private readonly ICOM_DOCUMENTO_FISICORepository _repoDoc;
		private readonly ICOM_PARAMETRORepository _repoParametro;
		
		public COM_SOLI_COTIZACION_DETA_DOCService(
			ICOM_SOLI_COTIZACION_DETA_DOCRepository repo,
		...
		public async Task<CResult> GetAllAsync(COM_SOLI_COTIZACION_DETA_DOCParam xWhere)
		{
		...
		public async Task<CResult> GetAsync(COM_SOLI_COTIZACION_DETA_DOCParam xWhere)
		{
			var p = new List<CParameter>
			{
				new CParameter() {ParameterName="CORR_EMPRESA",Value=xWhere.CORR_EMPRESA,DbType=System.Data.DbType.Int32},
		...
		public async Task<CResult> CreateAsync(COM_SOLI_COTIZACION_DETA_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.CreateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}
		
		public async Task<CResult> UpdateAsync(COM_SOLI_COTIZACION_DETA_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.UpdateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}
		
		public async Task<CResult> DeleteAsync(COM_SOLI_COTIZACION_DETA_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				File.Delete(((COM_SOLI_COTIZACION_DETA_DOCView)vResultado.Data).RUTA_DOCUMENTO);
			}
			catch (Exception e)
			{
		...
		public async Task<CResult> CreateDocAsync(COM_SOLI_COTIZACION_DETA_DOC_PDFTable Data, string pathRoot, string vLOGIN_SISTEMA, string vESTACION)
		{
			var vResultado = new CResult()
			{
				ErrorCode = 0,
			...
			// Guardar relación documento con solicitud
			if (vResultado.Result)
			{
				var Solidoc = new COM_SOLI_COTIZACION_DETA_DOCTable
				{
					CORR_EMPRESA = Data.CORR_EMPRESA,
					ANIO_PERIODO = Data.ANIO_PERIODO,
					CORR_SOLI_COTIZACION = Data.CORR_SOLI_COTIZACION,
					CORR_SOLI_COTIZACION_DETA = Data.CORR_SOLI_COTIZACION_DETA,
		...
		public async Task<Stream> GetDocAsync(COM_SOLI_COTIZACION_DETA_DOCParam xWhere)
		{
			...
			var p = new List<CParameter>
			{
				...
				new() {ParameterName="CORR_SOLI_COTIZACION_DETA",Value=xWhere.CORR_SOLI_COTIZACION_DETA,DbType=System.Data.DbType.Int32}
			};
			...
			if (vResultado.Result)
			{
				...
				{
					MemoryStream ms = new();
					using FileStream file = new(((COM_SOLI_COTIZACION_DETA_DOCView)vResultado.Data).RUTA_DOCUMENTO, FileMode.Open, FileAccess.Read);
