...
{
	public class COM_SOLI_COTIZACION_DOCService : ICOM_SOLI_COTIZACION_DOCService
	{
		private readonly ICOM_SOLI_COTIZACION_DOCRepository _repo;
		private readonly ICOM_DOCUMENTO_FISICORepository _repoDoc;
		private readonly ICOM_PARAMETRORepository _repoParametro;

		public COM_SOLI_COTIZACION_DOCService(
			ICOM_SOLI_COTIZACION_DOCRepository repo,
		...
		public async Task<CResult> GetAllAsync(COM_SOLI_COTIZACION_DOCParam xWhere)
		{
		...
		public async Task<CResult> GetAsync(COM_SOLI_COTIZACION_DOCParam xWhere)
		{
			var p = new List<CParameter>
			{
				new CParameter() {ParameterName="CORR_EMPRESA",Value=xWhere.CORR_EMPRESA,DbType=System.Data.DbType.Int32},
		...
		public async Task<CResult> CreateAsync(COM_SOLI_COTIZACION_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.CreateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}

		public async Task<CResult> UpdateAsync(COM_SOLI_COTIZACION_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.UpdateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}

		public async Task<CResult> DeleteAsync(COM_SOLI_COTIZACION_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				File.Delete(((COM_SOLI_COTIZACION_DOCView)vResultado.Data).RUTA_DOCUMENTO);
			}
			catch (Exception e)
			{
		...
		public async Task<CResult> CreateDocAsync(COM_SOLI_COTIZACION_DOC_PDF Data, string pathRoot, string vLOGIN_SISTEMA, string vESTACION)
		{
			var vResultado = new CResult()
			{
				ErrorCode = 0,
			...
			{
				var Solidoc = new COM_SOLI_COTIZACION_DOCTable
				{
					CORR_EMPRESA = Data.CORR_EMPRESA,
		...
		public async Task<Stream> GetDocAsync(COM_SOLI_COTIZACION_DOCParam xWhere)
		{
			...
			if (vResultado.Result)
			{
				...
				{
					MemoryStream ms = new();
					using FileStream file = new(((COM_SOLI_COTIZACION_DOCView)vResultado.Data).RUTA_DOCUMENTO, FileMode.Open, FileAccess.Read);
