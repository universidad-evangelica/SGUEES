...
{
	public class COM_COTIZACION_DOCService: ICOM_COTIZACION_DOCService
	{
		private readonly ICOM_COTIZACION_DOCRepository _repo;
		private readonly ICOM_DOCUMENTO_FISICORepository _repoDoc;
		private readonly ICOM_PARAMETRORepository _repoParametro;
		
		public COM_COTIZACION_DOCService(
			ICOM_COTIZACION_DOCRepository repo,
		...
		public async Task<CResult> GetAllAsync(COM_COTIZACION_DOCParam xWhere)
		{
		...
		public async Task<CResult> GetAsync(COM_COTIZACION_DOCParam xWhere)
		{
			var p = new List<CParameter>
			{
				new CParameter() {ParameterName="CORR_EMPRESA",Value=xWhere.CORR_EMPRESA,DbType=System.Data.DbType.Int32},
		...
		public async Task<CResult> CreateAsync(COM_COTIZACION_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.CreateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}
		
		public async Task<CResult> UpdateAsync(COM_COTIZACION_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.UpdateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}
		
		public async Task<CResult> DeleteAsync(COM_COTIZACION_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				File.Delete(((COM_COTIZACION_DOCView)vResultado.Data).RUTA_DOCUMENTO);
			}
			catch (Exception e)
			{
		...
		public async Task<CResult> CreateDocAsync(COM_COTIZACION_DOC_PDFTable Data, string pathRoot, string vLOGIN_SISTEMA, string vESTACION)
		{
			var vResultado = new CResult()
			{
				ErrorCode = 0,
			...
			{
				var Solidoc = new COM_COTIZACION_DOCTable
				{
					CORR_EMPRESA = Data.CORR_EMPRESA,
		...
		public async Task<Stream> GetDocAsync(COM_COTIZACION_DOCParam xWhere)
		{
			...
			{
				MemoryStream ms = new();
				using FileStream file = new(((COM_COTIZACION_DOCView)vResultado.Data).RUTA_DOCUMENTO, FileMode.Open, FileAccess.Read);
		...
		public async Task<CResult> GetAllCOM_CUADRO_COMPARATIVOAsync(COM_CUADRO_COMPARATIVOParam xWhere)
		{
			var p = new List<CParameter>
			{
				new() {ParameterName="CORR_EMPRESA",Value=xWhere.CORR_EMPRESA,DbType=System.Data.DbType.Int32},
			...
			return await _repo.GetAllCOM_CUADRO_COMPARATIVOAsync(p);
		}
	}
}
