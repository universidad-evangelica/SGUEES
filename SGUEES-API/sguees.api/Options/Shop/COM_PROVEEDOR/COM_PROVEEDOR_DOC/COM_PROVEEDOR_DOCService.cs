...
{
	public class COM_PROVEEDOR_DOCService: ICOM_PROVEEDOR_DOCService
	{
		private readonly ICOM_PROVEEDOR_DOCRepository _repo;
		private readonly ICOM_DOCUMENTO_FISICORepository _repoDoc;
		private readonly ICOM_PARAMETRORepository _repoParametro;
		
		public COM_PROVEEDOR_DOCService(
			ICOM_PROVEEDOR_DOCRepository repo,
		...
		public async Task<CResult> GetAllAsync(COM_PROVEEDOR_DOCParam xWhere)
		{
		...
		public async Task<CResult> GetAsync(COM_PROVEEDOR_DOCParam xWhere)
		{
			var p = new List<CParameter>
			{
				new CParameter() {ParameterName="CORR_EMPRESA",Value=xWhere.CORR_EMPRESA,DbType=System.Data.DbType.Int32},
		...
		public async Task<CResult> CreateAsync(COM_PROVEEDOR_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.CreateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}
		
		public async Task<CResult> UpdateAsync(COM_PROVEEDOR_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			return await _repo.UpdateAsync(Data, vLOGIN_SISTEMA, vESTACION);
		}
		
		public async Task<CResult> DeleteAsync(COM_PROVEEDOR_DOCTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				File.Delete(((COM_PROVEEDOR_DOCView)vResultado.Data).RUTA_DOCUMENTO);
			}
			catch (Exception e)
			{
		...
		public async Task<CResult> CreateDocAsync(COM_PROVEEDOR_DOC_PDFTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			var vResultado = new CResult()
			{
				ErrorCode = 0,
			...
			// Guardar relación documento con proveedor
			if (vResultado.Result)
			{
				var Solidoc = new COM_PROVEEDOR_DOCTable
				{
					CORR_EMPRESA = Data.CORR_EMPRESA,
					CORR_PROVEEDOR = Data.CORR_PROVEEDOR,
		...
		public async Task<Stream> GetDocAsync(COM_PROVEEDOR_DOCParam xWhere)
		{
			...
			if (vResultado.Result)
			{
				...
				{
					MemoryStream ms = new();
					using FileStream file = new(((COM_PROVEEDOR_DOCView)vResultado.Data).RUTA_DOCUMENTO, FileMode.Open, FileAccess.Read);
