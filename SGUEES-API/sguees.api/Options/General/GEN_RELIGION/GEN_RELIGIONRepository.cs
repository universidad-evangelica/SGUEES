// Qué hace: persistencia SQL del catálogo religión.
// Cómo lo hace: CRUD y ActivarInactivar sobre GEN_RELIGION / V_GEN_RELIGION (sin empresa).
using System;
...
{
	public class GEN_RELIGIONRepository : BaseRepository<GEN_RELIGIONTable>, IGEN_RELIGIONRepository
	{
		private const string _TableName = "GEN_RELIGION";
		private const string _ViewName = "V_GEN_RELIGION";
		private const string _CampoPk = "CORR_RELIGION";
		private const string _CampoEstado = "ACTIVO_RELIGION";
		private const bool _UsaEmpresa = false;

		public GEN_RELIGIONRepository(IConfiguration config) :
			base(config.GetConnectionString("defaultConnection"),
				config.GetSection("DbProvider:defaultProvider").Value)
		{
		}

		// Qué hace: lista todas las religiones.
		// Cómo lo hace: lee V_GEN_RELIGION y ordena por correlativo.
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<GEN_RELIGIONView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_RELIGION)
					.ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<GEN_RELIGIONView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_RELIGION ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> CreateAsync(GEN_RELIGIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_RELIGION", Value = Data.CORR_RELIGION, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "NOMBRE_RELIGION", Value = Data.NOMBRE_RELIGION, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "DESCRIPCION", Value = Data.DESCRIPCION, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "ACTIVO_RELIGION", Value = Data.ACTIVO_RELIGION ?? true, DbType = System.Data.DbType.Boolean },
		...
		// Qué hace: actualiza una religión.
		...
		public async Task<CResult> UpdateAsync(GEN_RELIGIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_RELIGION ?? Data.CORR_RELIGION;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> DeleteAsync(GEN_RELIGIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_RELIGION;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar la religión porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(GEN_RELIGIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_RELIGION, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<GEN_RELIGIONView>().FromDataReader(readerGet).FirstOrDefault();
