// Qué hace: persistencia SQL del catálogo parentesco.
// Cómo lo hace: CRUD y ActivarInactivar sobre GEN_PARENTESCO / V_GEN_PARENTESCO (sin empresa).
using System;
...
{
	public class GEN_PARENTESCORepository : BaseRepository<GEN_PARENTESCOTable>, IGEN_PARENTESCORepository
	{
		private const string _TableName = "GEN_PARENTESCO";
		private const string _ViewName = "V_GEN_PARENTESCO";
		private const string _CampoPk = "CORR_PARENTESCO";
		private const string _CampoEstado = "ACTIVO_PARENTESCO";
		private const bool _UsaEmpresa = false;

		public GEN_PARENTESCORepository(IConfiguration config) :
		...
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<GEN_PARENTESCOView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_PARENTESCO)
					.ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<GEN_PARENTESCOView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_PARENTESCO ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> CreateAsync(GEN_PARENTESCOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_PARENTESCO", Value = Data.CORR_PARENTESCO, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "NOMBRE_PARENTESCO", Value = Data.NOMBRE_PARENTESCO, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "DESCRIPCION", Value = Data.DESCRIPCION, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "ACTIVO_PARENTESCO", Value = Data.ACTIVO_PARENTESCO ?? true, DbType = System.Data.DbType.Boolean },
		...
		public async Task<CResult> UpdateAsync(GEN_PARENTESCOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				var pWhere = new List<CParameter>
				{
					new CParameter() { ParameterName = "CORR_PARENTESCO", Value = Data.CORR_PARENTESCO, DbType = System.Data.DbType.Int32 },
				};
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_PARENTESCO ?? Data.CORR_PARENTESCO;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> DeleteAsync(GEN_PARENTESCOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_PARENTESCO;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar el parentesco porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(GEN_PARENTESCOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_PARENTESCO, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<GEN_PARENTESCOView>().FromDataReader(readerGet).FirstOrDefault();
