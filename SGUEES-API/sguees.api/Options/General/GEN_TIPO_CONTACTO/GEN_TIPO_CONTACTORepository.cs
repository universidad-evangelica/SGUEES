// Qué hace: persistencia SQL del catálogo tipo contacto.
// Cómo lo hace: CRUD y ActivarInactivar sobre GEN_TIPO_CONTACTO / V_GEN_TIPO_CONTACTO (sin empresa).
using System;
...
{
	public class GEN_TIPO_CONTACTORepository : BaseRepository<GEN_TIPO_CONTACTOTable>, IGEN_TIPO_CONTACTORepository
	{
		private const string _TableName = "GEN_TIPO_CONTACTO";
		private const string _ViewName = "V_GEN_TIPO_CONTACTO";
		private const string _CampoPk = "CORR_TIPO_CONTACTO";
		private const string _CampoEstado = "ACTIVO_TIPO_CONTACTO";
		private const bool _UsaEmpresa = false;

		public GEN_TIPO_CONTACTORepository(IConfiguration config) :
		...
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<GEN_TIPO_CONTACTOView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_TIPO_CONTACTO)
					.ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<GEN_TIPO_CONTACTOView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_TIPO_CONTACTO ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> CreateAsync(GEN_TIPO_CONTACTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_TIPO_CONTACTO", Value = Data.CORR_TIPO_CONTACTO, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "NOMBRE_TIPO_CONTACTO", Value = Data.NOMBRE_TIPO_CONTACTO, DbType = System.Data.DbType.String },
		...
		public async Task<CResult> UpdateAsync(GEN_TIPO_CONTACTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				var pWhere = new List<CParameter>
				{
					new CParameter() { ParameterName = "CORR_TIPO_CONTACTO", Value = Data.CORR_TIPO_CONTACTO, DbType = System.Data.DbType.Int32 },
				};
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_TIPO_CONTACTO ?? Data.CORR_TIPO_CONTACTO;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> DeleteAsync(GEN_TIPO_CONTACTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_TIPO_CONTACTO;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar el tipo de contacto porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(GEN_TIPO_CONTACTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_TIPO_CONTACTO, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<GEN_TIPO_CONTACTOView>().FromDataReader(readerGet).FirstOrDefault();
