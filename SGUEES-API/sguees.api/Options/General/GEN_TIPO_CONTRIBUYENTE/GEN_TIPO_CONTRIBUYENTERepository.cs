// Qué hace: persistencia SQL del catálogo tipo contribuyente.
// Cómo lo hace: CRUD y ActivarInactivar sobre GEN_TIPO_CONTRIBUYENTE / V_GEN_TIPO_CONTRIBUYENTE (sin empresa).
using System;
...
{
	public class GEN_TIPO_CONTRIBUYENTERepository : BaseRepository<GEN_TIPO_CONTRIBUYENTETable>, IGEN_TIPO_CONTRIBUYENTERepository
	{
		private const string _TableName = "GEN_TIPO_CONTRIBUYENTE";
		private const string _ViewName = "V_GEN_TIPO_CONTRIBUYENTE";
		private const string _CampoPk = "CORR_TIPO_CONTRIBUYENTE";
		private const string _CampoEstado = "ACTIVO_TIPO_CONTRIBUYENTE";
		private const bool _UsaEmpresa = false;

		public GEN_TIPO_CONTRIBUYENTERepository(IConfiguration config) :
			base(config.GetConnectionString("defaultConnection"),
				config.GetSection("DbProvider:defaultProvider").Value)
		{
		}

		// Qué hace: lista todos los tipos de contribuyente.
		// Cómo lo hace: lee V_GEN_TIPO_CONTRIBUYENTE y ordena por correlativo.
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<GEN_TIPO_CONTRIBUYENTEView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_TIPO_CONTRIBUYENTE)
					.ToList();
		...
		// Qué hace: obtiene un tipo de contribuyente.
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<GEN_TIPO_CONTRIBUYENTEView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_TIPO_CONTRIBUYENTE ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		// Qué hace: inserta un tipo de contribuyente.
		// Cómo lo hace: Insert con OUTPUT/re-read de la vista; controla llave duplicada.
		public async Task<CResult> CreateAsync(GEN_TIPO_CONTRIBUYENTETable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_TIPO_CONTRIBUYENTE", Value = Data.CORR_TIPO_CONTRIBUYENTE, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "NOMBRE_TIPO_CONTRIBUYENTE", Value = Data.NOMBRE_TIPO_CONTRIBUYENTE, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "ACTIVO_TIPO_CONTRIBUYENTE", Value = Data.ACTIVO_TIPO_CONTRIBUYENTE ?? true, DbType = System.Data.DbType.Boolean },
		...
		// Qué hace: actualiza un tipo de contribuyente.
		// Cómo lo hace: Update por PK y relee la vista; controla llave duplicada.
		public async Task<CResult> UpdateAsync(GEN_TIPO_CONTRIBUYENTETable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			...
			try
			{
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_TIPO_CONTRIBUYENTE ?? Data.CORR_TIPO_CONTRIBUYENTE;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		// Qué hace: elimina un tipo de contribuyente.
		// Cómo lo hace: Delete por PK; ante FK informa registros asociados.
		public async Task<CResult> DeleteAsync(GEN_TIPO_CONTRIBUYENTETable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_TIPO_CONTRIBUYENTE;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar el tipo de contribuyente porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(GEN_TIPO_CONTRIBUYENTETable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_TIPO_CONTRIBUYENTE, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<GEN_TIPO_CONTRIBUYENTEView>().FromDataReader(readerGet).FirstOrDefault();
