...
namespace sguees.Repositories
{
	public class GEN_DIVISIONRepository : BaseRepository<GEN_DIVISIONTable>, IGEN_DIVISIONRepository
	{
		...
		public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
				{
					...
					objResultado.ErrorSource = "[GEN_DIVISIONRepository]";
					return objResultado;
				}
		...
		private async Task<List<GEN_DIVISIONView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
		{
			var dbWhere = xWhere
			...
			var response = new List<GEN_DIVISIONView>().FromDataReader(reader).ToList();
		...
		private static List<GEN_DIVISIONView> ApplyColumnFilters(
			List<GEN_DIVISIONView> response,
		...
		private static List<GEN_DIVISIONView> ApplySort(List<GEN_DIVISIONView> response, List<CParameter> xWhere)
		{
			...
			IEnumerable<GEN_DIVISIONView> ordered = sortField switch
			{
				"CORR_DIVISION" => desc
				...
				_ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_DIVISION),
			};
		...
		private static bool MatchesExactColumnValue(GEN_DIVISIONView row, string columnName, string filterValue)
		{
		...
		private static bool MatchesAnyOfColumnValue(GEN_DIVISIONView row, string columnName, string filterValue)
		{
			if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
			{
				return string.IsNullOrWhiteSpace(GetColumnValue(row, columnName));
		...
		{
			return string.Equals(columnName, "CORR_DIVISION", StringComparison.OrdinalIgnoreCase);
		}
		...
		private static object GetDistinctValue(GEN_DIVISIONView row, string columnName)
		{
			switch (columnName)
			{
				case "CORR_DIVISION":
		...
		private static string GetColumnValue(GEN_DIVISIONView row, string columnName)
		{
			switch (columnName)
			{
				case "CORR_DIVISION":
