// Qué hace: persistencia SQL del catálogo actividad económica.
// Cómo lo hace: CRUD y ActivarInactivar sobre GEN_ACTIVIDAD_ECONOMICA / V_GEN_ACTIVIDAD_ECONOMICA (sin empresa).
using System;
...
{
	public class GEN_ACTIVIDAD_ECONOMICARepository : BaseRepository<GEN_ACTIVIDAD_ECONOMICATable>, IGEN_ACTIVIDAD_ECONOMICARepository
	{
		private const string _TableName = "GEN_ACTIVIDAD_ECONOMICA";
		private const string _ViewName = "V_GEN_ACTIVIDAD_ECONOMICA";
		private const string _CampoPk = "CORR_ACTIVIDAD_ECONOMICA";
		private const string _CampoEstado = "ACTIVO_ACTIVIDAD_ECONOMICA";
		private const bool _UsaEmpresa = false;

		public GEN_ACTIVIDAD_ECONOMICARepository(IConfiguration config) :
			base(config.GetConnectionString("defaultConnection"),
				config.GetSection("DbProvider:defaultProvider").Value)
		{
		}

		// Qué hace: lista todas las actividades económicas.
		// Cómo lo hace: lee V_GEN_ACTIVIDAD_ECONOMICA y ordena por correlativo.
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<GEN_ACTIVIDAD_ECONOMICAView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_ACTIVIDAD_ECONOMICA)
					.ToList();
		...
		// Qué hace: obtiene una actividad económica.
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<GEN_ACTIVIDAD_ECONOMICAView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_ACTIVIDAD_ECONOMICA ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		// Qué hace: inserta una actividad económica.
		...
		public async Task<CResult> CreateAsync(GEN_ACTIVIDAD_ECONOMICATable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_ACTIVIDAD_ECONOMICA", Value = Data.CORR_ACTIVIDAD_ECONOMICA, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "CODIGO_ACTIVIDAD_ECONOMICA", Value = Data.CODIGO_ACTIVIDAD_ECONOMICA, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "NOMBRE_ACTIVIDAD_ECONOMICA", Value = Data.NOMBRE_ACTIVIDAD_ECONOMICA, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "ACTIVO_ACTIVIDAD_ECONOMICA", Value = Data.ACTIVO_ACTIVIDAD_ECONOMICA ?? true, DbType = System.Data.DbType.Boolean },
		...
		// Qué hace: actualiza una actividad económica.
		...
		public async Task<CResult> UpdateAsync(GEN_ACTIVIDAD_ECONOMICATable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_ACTIVIDAD_ECONOMICA ?? Data.CORR_ACTIVIDAD_ECONOMICA;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		// Qué hace: elimina una actividad económica.
		...
		public async Task<CResult> DeleteAsync(GEN_ACTIVIDAD_ECONOMICATable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_ACTIVIDAD_ECONOMICA;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar la actividad económica porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(GEN_ACTIVIDAD_ECONOMICATable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_ACTIVIDAD_ECONOMICA, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<GEN_ACTIVIDAD_ECONOMICAView>().FromDataReader(readerGet).FirstOrDefault();
