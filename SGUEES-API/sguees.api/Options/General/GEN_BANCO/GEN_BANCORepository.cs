...
namespace sguees.Repositories
{
	public class GEN_BANCORepository : BaseRepository<GEN_BANCOTable>, IGEN_BANCORepository
	{
		...
		private const string _ViewName = "V_GEN_BANCO";
		...
		public async Task<CResult> ActivarInactivarAsync(GEN_BANCOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			...
			try
			{
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<GEN_BANCOView>().FromDataReader(readerGet).FirstOrDefault();
