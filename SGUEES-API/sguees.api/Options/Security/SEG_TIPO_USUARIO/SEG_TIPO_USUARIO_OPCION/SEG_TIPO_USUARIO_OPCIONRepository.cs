...
namespace scuees.Repositories
{
    ...
    {
        private const string _TableName = "SEG_TIPO_USUARIO_OPCION";

        public SEG_TIPO_USUARIO_OPCIONRepository(IConfiguration config) : 
                base(config.GetConnectionString("defaultConnection"), 
                     config.GetSection("DbProvider:defaultProvider").Value)
        {
        }

        public Task<CResult> CreateAsync(SEG_TIPO_USUARIO_OPCIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            throw new System.NotImplementedException();
        }

        public Task<CResult> DeleteAsync(SEG_TIPO_USUARIO_OPCIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            throw new System.NotImplementedException();
        }
        ...
        public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
            ...
            try
            {
                ...
                var response = new List<SEG_TIPO_USUARIO_OPCIONView>().FromDataReader(reader).ToList();
        ...
        public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
            ...
            try
            {
			    ...
			    var response = new List<SEG_TIPO_USUARIO_OPCIONView>().FromDataReader(reader).FirstOrDefault();
                                                    ...
                                                    string CODIGO_SUITE)
		{
            ...
            try
            {
                ...
                BEGIN
                    ...
                    WHERE A.TIPO_USUARIO=2 --Para usuarios que se autentican desde CLASS
                    AND EXISTS(SELECT 1 FROM SEG_OPCION_SISTEMA_SUITE B
                            WHERE A.CODIGO_OPCION=B.CODIGO_OPCION
                            AND LTRIM(RTRIM(UPPER(B.CODIGO_SUITE)))=LTRIM(RTRIM(UPPER(@CODIGO_SUITE))))
                    ORDER BY A.ORDEN_SISTEMA DESC,A.ORDEN_MENU,A.ORDEN_OPCION
        ...
        public async Task<CResult> UpdateAsync(SEG_TIPO_USUARIO_OPCIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            CResult objResultado = new CResult();
