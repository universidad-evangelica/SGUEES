// Qué hace: persistencia SQL del catálogo AFP.
// Cómo lo hace: CRUD y ActivarInactivar sobre PLA_AFP / V_PLA_AFP (sin empresa).
using System;
...
{
	public class PLA_AFPRepository : BaseRepository<PLA_AFPTable>, IPLA_AFPRepository
	{
		private const string _TableName = "PLA_AFP";
		private const string _ViewName = "V_PLA_AFP";
		private const string _CampoPk = "CORR_AFP";
		private const string _CampoEstado = "ACTIVO_AFP";
		private const bool _UsaEmpresa = false;

		public PLA_AFPRepository(IConfiguration config) :
		...
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<PLA_AFPView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_AFP)
					.ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<PLA_AFPView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_AFP ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> CreateAsync(PLA_AFPTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_AFP", Value = Data.CORR_AFP, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "NOMBRE_AFP", Value = Data.NOMBRE_AFP, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "NOMBRE_CORTO_AFP", Value = Data.NOMBRE_CORTO_AFP, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "CODIGO_SGVPP", Value = Data.CODIGO_SGVPP, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "INCLUYE_SEPP", Value = Data.INCLUYE_SEPP ?? false, DbType = System.Data.DbType.Boolean },
					new CParameter() { ParameterName = "ACTIVO_AFP", Value = Data.ACTIVO_AFP ?? true, DbType = System.Data.DbType.Boolean },
		...
		public async Task<CResult> UpdateAsync(PLA_AFPTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_AFP ?? Data.CORR_AFP;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> DeleteAsync(PLA_AFPTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_AFP;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar la AFP porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(PLA_AFPTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_AFP, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<PLA_AFPView>().FromDataReader(readerGet).FirstOrDefault();
