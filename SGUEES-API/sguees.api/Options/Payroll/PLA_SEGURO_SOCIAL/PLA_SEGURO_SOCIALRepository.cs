// Qué hace: persistencia SQL del catálogo Seguro Social.
// Cómo lo hace: CRUD y ActivarInactivar sobre PLA_SEGURO_SOCIAL / V_PLA_SEGURO_SOCIAL (sin empresa).
using System;
...
{
	public class PLA_SEGURO_SOCIALRepository : BaseRepository<PLA_SEGURO_SOCIALTable>, IPLA_SEGURO_SOCIALRepository
	{
		private const string _TableName = "PLA_SEGURO_SOCIAL";
		private const string _ViewName = "V_PLA_SEGURO_SOCIAL";
		private const string _CampoPk = "CORR_SEGURO_SOCIAL";
		private const string _CampoEstado = "ACTIVO_SEGURO_SOCIAL";
		private const bool _UsaEmpresa = false;

		public PLA_SEGURO_SOCIALRepository(IConfiguration config) :
		...
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<PLA_SEGURO_SOCIALView>().FromDataReader(reader)
					.OrderBy(x => x.CORR_SEGURO_SOCIAL)
					.ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader(_ViewName, xWhere);
				var response = new List<PLA_SEGURO_SOCIALView>().FromDataReader(reader).FirstOrDefault();
				...
				objResultado.CodeHelper = response?.CORR_SEGURO_SOCIAL ?? 0;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> CreateAsync(PLA_SEGURO_SOCIALTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				{
					new CParameter() { ParameterName = "CORR_SEGURO_SOCIAL", Value = Data.CORR_SEGURO_SOCIAL, DbType = System.Data.DbType.Int32, Direction = System.Data.ParameterDirection.InputOutput },
					new CParameter() { ParameterName = "NOMBRE_SEGURO_SOCIAL", Value = Data.NOMBRE_SEGURO_SOCIAL, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "NOMBRE_CORTO_SEGURO", Value = Data.NOMBRE_CORTO_SEGURO, DbType = System.Data.DbType.String },
					new CParameter() { ParameterName = "ACTIVO_SEGURO_SOCIAL", Value = Data.ACTIVO_SEGURO_SOCIAL ?? true, DbType = System.Data.DbType.Boolean },
		...
		public async Task<CResult> UpdateAsync(PLA_SEGURO_SOCIALTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Result = true;
				objResultado.RowsAffected = 1;
				objResultado.CodeHelper = response?.CORR_SEGURO_SOCIAL ?? Data.CORR_SEGURO_SOCIAL;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> DeleteAsync(PLA_SEGURO_SOCIALTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				...
				objResultado.Data = null;
				objResultado.Result = true;
				objResultado.CodeHelper = Data.CORR_SEGURO_SOCIAL;
				objResultado.ErrorCode = 0;
				objResultado.ErrorMessage = "";
				objResultado.ErrorSource = "";
			}
			catch (Exception e)
			{
				...
				objResultado.CodeHelper = 0;
				objResultado.ErrorCode = -1;
				objResultado.ErrorMessage = "No se puede eliminar el seguro social porque tiene registros asociados en otras tablas.";
				objResultado.ErrorSource += $"[{e.Source}]";
			}
			finally
			{
		...
		public async Task<CResult> ActivarInactivarAsync(PLA_SEGURO_SOCIALTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			...
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() { ParameterName = "CORR_RELATIVO", Value = Data.CORR_SEGURO_SOCIAL, DbType = System.Data.DbType.Int32 },
				...
				if ((int)objData.objCommand.Parameters["@SYS_NUMERO_ERROR"].Value == 0)
				{
					...
					var response = new List<PLA_SEGURO_SOCIALView>().FromDataReader(readerGet).FirstOrDefault();
