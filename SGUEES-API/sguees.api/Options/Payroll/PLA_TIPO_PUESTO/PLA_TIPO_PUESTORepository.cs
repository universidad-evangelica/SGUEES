...
namespace SGUEES.Repositories
{
    public class PLA_TIPO_PUESTORepository : BaseRepository<PLA_TIPO_PUESTOTable>, IPLA_TIPO_PUESTORepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[PLA_TIPO_PUESTORepository]";
                    return objResultado;
                }
        ...
        private async Task<List<PLA_TIPO_PUESTOView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
        {
        ...
        private static List<PLA_TIPO_PUESTOView> ApplyColumnFilters(
            List<PLA_TIPO_PUESTOView> response,
        ...
        private static List<PLA_TIPO_PUESTOView> ApplySort(List<PLA_TIPO_PUESTOView> response, List<CParameter> xWhere)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortField) || !IsAllowedDistinctField(sortField))
            {
                return response
                    ...
                    .ThenBy(x => x.CORR_TIPO_PUESTO)
                    .ToList();
            }

            var desc = sortDescValue ?? false;
            IEnumerable<PLA_TIPO_PUESTOView> ordered = sortField switch
            {
                "CORR_TIPO_PUESTO" => desc
                    ? response.OrderByDescending(x => x.CORR_TIPO_PUESTO)
                    : response.OrderBy(x => x.CORR_TIPO_PUESTO),
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_TIPO_PUESTO),
            };
        ...
        private static bool MatchesExactColumnValue(PLA_TIPO_PUESTOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_TIPO_PUESTO", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(PLA_TIPO_PUESTOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_TIPO_PUESTO", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(PLA_TIPO_PUESTOView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                return row.ESTADO_TIPO_PUESTO.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_TIPO_PUESTO", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static object GetDistinctValue(PLA_TIPO_PUESTOView row, string columnName)
        {
            switch (columnName)
            {
        ...
        private static string GetColumnValue(PLA_TIPO_PUESTOView row, string columnName)
        {
            switch (columnName)
            {
                ...
                case "ESTADO_TIPO_PUESTO":
                    ...
                    return row.ESTADO_TIPO_PUESTO.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
