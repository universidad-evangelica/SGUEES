...
namespace SGUEES.Repositories
{
    public class PLA_NIVEL_ACADEMICORepository : BaseRepository<PLA_NIVEL_ACADEMICOTable>, IPLA_NIVEL_ACADEMICORepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[PLA_NIVEL_ACADEMICORepository]";
                    return objResultado;
                }
        ...
        private async Task<List<PLA_NIVEL_ACADEMICOView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
        {
            var dbWhere = xWhere
            ...
            var estado = xWhere
                .Where(x => x.ParameterName == "ESTADO_NIVEL_ACADEMICO")
            ...
            var response = new List<PLA_NIVEL_ACADEMICOView>().FromDataReader(reader).ToList();
            ...
            {
                response = response
                    .Where(x => (x.ESTADO_NIVEL_ACADEMICO ?? false) == estado.Value)
                    .ToList();
            }
            ...
            if (!string.IsNullOrWhiteSpace(busqueda))
            {
                ...
                response = response
                    .Where(x =>
                        ...
                        Contains(x.CORR_NIVEL_ACADEMICO.ToString(), search) ||
                        Contains(x.NOMBRE_NIVEL_ACADEMICO, search) ||
                        Contains((x.ESTADO_NIVEL_ACADEMICO ?? false) ? "Activo" : "Inactivo", search) ||
        ...
        private static List<PLA_NIVEL_ACADEMICOView> ApplyColumnFilters(
            List<PLA_NIVEL_ACADEMICOView> response,
        ...
        private static List<PLA_NIVEL_ACADEMICOView> ApplySort(List<PLA_NIVEL_ACADEMICOView> response, List<CParameter> xWhere)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortField) || !IsAllowedDistinctField(sortField))
            {
                return response
                    ...
                    .ThenBy(x => x.CORR_NIVEL_ACADEMICO)
                    .ToList();
            }

            var desc = sortDescValue ?? false;
            IEnumerable<PLA_NIVEL_ACADEMICOView> ordered = sortField switch
            {
                "CORR_NIVEL_ACADEMICO" => desc
                    ? response.OrderByDescending(x => x.CORR_NIVEL_ACADEMICO)
                    : response.OrderBy(x => x.CORR_NIVEL_ACADEMICO),
                "NOMBRE_NIVEL_ACADEMICO" => desc
                    ? response.OrderByDescending(x => x.NOMBRE_NIVEL_ACADEMICO)
                    : response.OrderBy(x => x.NOMBRE_NIVEL_ACADEMICO),
                "ESTADO_NIVEL_ACADEMICO" => desc
                    ? response.OrderByDescending(x => x.ESTADO_NIVEL_ACADEMICO ?? false)
                    : response.OrderBy(x => x.ESTADO_NIVEL_ACADEMICO ?? false),
                "USUARIO_CREA" => desc
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_NIVEL_ACADEMICO),
            };
        ...
        private static bool IsAllowedDistinctField(string field)
        {
            return field switch
            {
                "CORR_NIVEL_ACADEMICO" => true,
                "NOMBRE_NIVEL_ACADEMICO" => true,
        ...
        private static bool MatchesExactColumnValue(PLA_NIVEL_ACADEMICOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_NIVEL_ACADEMICO", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(PLA_NIVEL_ACADEMICOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_NIVEL_ACADEMICO", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(PLA_NIVEL_ACADEMICOView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
                return !row.ESTADO_NIVEL_ACADEMICO.HasValue;
            }

            if (!row.ESTADO_NIVEL_ACADEMICO.HasValue)
            {
                return false;
            }
            ...
            {
                return row.ESTADO_NIVEL_ACADEMICO.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_NIVEL_ACADEMICO", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static List<object> CollectDistinctValuesInRowOrder(
            List<PLA_NIVEL_ACADEMICOView> rows,
        ...
        private static object GetDistinctValue(PLA_NIVEL_ACADEMICOView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_NIVEL_ACADEMICO":
                    return row.CORR_NIVEL_ACADEMICO;
                case "NOMBRE_NIVEL_ACADEMICO":
                    return row.NOMBRE_NIVEL_ACADEMICO;
                case "ESTADO_NIVEL_ACADEMICO":
                    return row.ESTADO_NIVEL_ACADEMICO.HasValue
                        ? row.ESTADO_NIVEL_ACADEMICO.Value
                        : null;
                case "USUARIO_CREA":
        ...
        private static string GetColumnValue(PLA_NIVEL_ACADEMICOView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_NIVEL_ACADEMICO":
                    return row.CORR_NIVEL_ACADEMICO.ToString();
                case "NOMBRE_NIVEL_ACADEMICO":
                    return row.NOMBRE_NIVEL_ACADEMICO;
                case "ESTADO_NIVEL_ACADEMICO":
                    if (!row.ESTADO_NIVEL_ACADEMICO.HasValue)
                    ...
                    return row.ESTADO_NIVEL_ACADEMICO.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
        ...
        public async Task<CResult> CreateAsync(PLA_NIVEL_ACADEMICOTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            ...
            try
            {
                ...
                var reader = await objData.Insert(_TableName, p, "CORR_NIVEL_ACADEMICO", pWhere);
                var response = new List<PLA_NIVEL_ACADEMICOView>().FromDataReader(reader).FirstOrDefault();
        ...
        public async Task<CResult> DeleteAsync(PLA_NIVEL_ACADEMICOTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            ...
            catch (Exception e)
            {
                ...
                objResultado.CodeHelper = 0;
                objResultado.ErrorCode = -1;
                objResultado.ErrorMessage = "No se puede eliminar el nivel academico porque tiene registros asociados.";
                objResultado.ErrorSource += $"[{e.Source}]";
            }
            finally
            {
