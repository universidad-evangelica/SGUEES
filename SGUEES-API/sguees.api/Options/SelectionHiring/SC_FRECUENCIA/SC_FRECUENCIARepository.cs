...
namespace SGUEES.Repositories
{
    public class SC_FRECUENCIARepository : BaseRepository<SC_FRECUENCIATable>, ISC_FRECUENCIARepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[SC_FRECUENCIARepository]";
                    return objResultado;
                }
        ...
        private static List<SC_FRECUENCIAView> ApplyColumnFilters(
            List<SC_FRECUENCIAView> response,
        ...
        private static List<SC_FRECUENCIAView> ApplySort(List<SC_FRECUENCIAView> response, List<CParameter> xWhere)
        {
            ...
            IEnumerable<SC_FRECUENCIAView> ordered = sortField switch
            {
                "CORR_FRECUENCIA" => desc
                    ? response.OrderByDescending(x => x.CORR_FRECUENCIA)
                    : response.OrderBy(x => x.CORR_FRECUENCIA),
                "NOMBRE_FRECUENCIA" => desc
                    ? response.OrderByDescending(x => x.NOMBRE_FRECUENCIA)
                    : response.OrderBy(x => x.NOMBRE_FRECUENCIA),
                "ESTADO_FRECUENCIA" => desc
                    ? response.OrderByDescending(x => x.ESTADO_FRECUENCIA ?? false)
                    : response.OrderBy(x => x.ESTADO_FRECUENCIA ?? false),
                "USUARIO_CREA" => desc
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_FRECUENCIA),
            };
        ...
        private static bool IsAllowedDistinctField(string field)
        {
            return field switch
            {
                "CORR_FRECUENCIA" => true,
                "NOMBRE_FRECUENCIA" => true,
        ...
        private static bool MatchesExactColumnValue(SC_FRECUENCIAView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_FRECUENCIA", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(SC_FRECUENCIAView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_FRECUENCIA", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(SC_FRECUENCIAView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
                return !row.ESTADO_FRECUENCIA.HasValue;
            }

            if (!row.ESTADO_FRECUENCIA.HasValue)
            {
                return false;
            }
            ...
            {
                return row.ESTADO_FRECUENCIA.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_FRECUENCIA", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static object GetDistinctValue(SC_FRECUENCIAView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_FRECUENCIA":
                    return row.CORR_FRECUENCIA;
                case "NOMBRE_FRECUENCIA":
                    return row.NOMBRE_FRECUENCIA;
                case "ESTADO_FRECUENCIA":
                    return row.ESTADO_FRECUENCIA.HasValue
                        ? row.ESTADO_FRECUENCIA.Value
                        : null;
                case "USUARIO_CREA":
        ...
        private static string GetColumnValue(SC_FRECUENCIAView row, string columnName)
        {
            switch (columnName)
            {
                ...
                case "ESTADO_FRECUENCIA":
                    ...
                    return row.ESTADO_FRECUENCIA.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
        ...
        public async Task<bool> ExistsNombreAsync(int corrEmpresa, string nombre, int excludeCorr)
        {
            ...
            const string sql = @"SELECT TOP 1 1 AS FOUND
                FROM V_SC_FRECUENCIA
                WHERE CORR_EMPRESA = @CORR_EMPRESA
                AND UPPER(LTRIM(RTRIM(NOMBRE_FRECUENCIA))) = UPPER(LTRIM(RTRIM(@NOMBRE)))
                AND (@EXCLUDE_CORR <= 0 OR CORR_FRECUENCIA <> @EXCLUDE_CORR)";
