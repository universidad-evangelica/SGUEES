...
{
    public class SC_DESCRIPTOR_RIESGO_PUESTOService : ISC_DESCRIPTOR_RIESGO_PUESTOService
    {
        private readonly ISC_DESCRIPTOR_RIESGO_PUESTORepository _repo;

        public SC_DESCRIPTOR_RIESGO_PUESTOService(ISC_DESCRIPTOR_RIESGO_PUESTORepository repo)
        {
            _repo = repo;
        }

        public async Task<CResult> GetAllAsync(SC_DESCRIPTOR_RIESGO_PUESTOParam xWhere)
        {
        ...
        public async Task<CResult> GetAsync(SC_DESCRIPTOR_RIESGO_PUESTOParam xWhere)
        {
            var p = new List<CParameter>
            {
                new CParameter() { ParameterName = "CORR_EMPRESA", Value = xWhere.CORR_EMPRESA, DbType = System.Data.DbType.Int32 },
        ...
        public async Task<CResult> CreateAsync(SC_DESCRIPTOR_RIESGO_PUESTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
        ...
        public async Task<CResult> UpdateAsync(SC_DESCRIPTOR_RIESGO_PUESTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            var validation = Validate(Data);
            if (validation != null) return validation;
        ...
        public async Task<CResult> DeleteAsync(SC_DESCRIPTOR_RIESGO_PUESTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            return await _repo.DeleteAsync(Data, vLOGIN_SISTEMA, vESTACION);
        }

        public async Task<CResult> DesactivarAsync(SC_DESCRIPTOR_RIESGO_PUESTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            Data.ESTADO_RIESGO_PUESTO = false;
            return await _repo.UpdateAsync(Data, vLOGIN_SISTEMA, vESTACION);
        }

        private static CResult Validate(SC_DESCRIPTOR_RIESGO_PUESTOTable Data)
        {
            if (Data == null)
            {
        ...
        private static CResult ValidationError(string message)
        {
            return new CResult
            {
                ...
                ErrorSource = "[SC_DESCRIPTOR_RIESGO_PUESTOService]",
                RowsAffected = 0
            };
        }
    }
