...
namespace SGUEES.Repositories
{
    public class SC_INDUCCIONRepository : BaseRepository<SC_INDUCCIONTable>, ISC_INDUCCIONRepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[SC_INDUCCIONRepository]";
                    return objResultado;
                }
        ...
        private async Task<List<SC_INDUCCIONView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
        {
            var dbWhere = xWhere
            ...
            var estado = xWhere
                .Where(x => x.ParameterName == "ESTADO_INDUCCION")
            ...
            {
                response = response
                    .Where(x => (x.ESTADO_INDUCCION ?? false) == estado.Value)
                    .ToList();
            }
        ...
        private static List<SC_INDUCCIONView> ApplyColumnFilters(
            List<SC_INDUCCIONView> response,
        ...
        private static List<SC_INDUCCIONView> ApplySort(List<SC_INDUCCIONView> response, List<CParameter> xWhere)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortField) || !IsAllowedDistinctField(sortField))
            {
                return response
                    ...
                    .ThenBy(x => x.CORR_INDUCCION)
                    .ToList();
            }

            var desc = sortDescValue ?? false;
            IEnumerable<SC_INDUCCIONView> ordered = sortField switch
            {
                "CORR_INDUCCION" => desc
                    ? response.OrderByDescending(x => x.CORR_INDUCCION)
                    : response.OrderBy(x => x.CORR_INDUCCION),
                "NOMBRE_INDUCCION" => desc
                    ? response.OrderByDescending(x => x.NOMBRE_INDUCCION)
                    : response.OrderBy(x => x.NOMBRE_INDUCCION),
                "SEMANAS_INDUCCION" => desc
                    ? response.OrderByDescending(x => x.SEMANAS_INDUCCION)
                    : response.OrderBy(x => x.SEMANAS_INDUCCION),
                "ESTADO_INDUCCION" => desc
                    ? response.OrderByDescending(x => x.ESTADO_INDUCCION ?? false)
                    : response.OrderBy(x => x.ESTADO_INDUCCION ?? false),
                "USUARIO_CREA" => desc
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_INDUCCION),
            };
        ...
        private static bool IsAllowedDistinctField(string field)
        {
            ...
            {
                "CORR_INDUCCION" => true,
                "NOMBRE_INDUCCION" => true,
                "SEMANAS_INDUCCION" => true,
        ...
        private static bool MatchesExactColumnValue(SC_INDUCCIONView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_INDUCCION", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(SC_INDUCCIONView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_INDUCCION", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(SC_INDUCCIONView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
                return !row.ESTADO_INDUCCION.HasValue;
            }

            if (!row.ESTADO_INDUCCION.HasValue)
            {
                return false;
            }
            ...
            {
                return row.ESTADO_INDUCCION.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_INDUCCION", StringComparison.OrdinalIgnoreCase)
                || string.Equals(columnName, "SEMANAS_INDUCCION", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static object GetDistinctValue(SC_INDUCCIONView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_INDUCCION":
                    return row.CORR_INDUCCION;
                case "NOMBRE_INDUCCION":
                    return row.NOMBRE_INDUCCION;
                case "SEMANAS_INDUCCION":
                    return row.SEMANAS_INDUCCION;
                case "ESTADO_INDUCCION":
                    return row.ESTADO_INDUCCION.HasValue
                        ? row.ESTADO_INDUCCION.Value
                        : null;
                case "USUARIO_CREA":
        ...
        private static string GetColumnValue(SC_INDUCCIONView row, string columnName)
        {
            switch (columnName)
            {
                ...
                case "ESTADO_INDUCCION":
                    ...
                    return row.ESTADO_INDUCCION.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
        ...
        public async Task<CResult> DeleteAsync(SC_INDUCCIONTable Data, string vLOGIN_SISTEMA, string vESTACION)
        {
            ...
            catch (Exception e)
            {
                ...
                objResultado.CodeHelper = 0;
                objResultado.ErrorCode = -1;
                objResultado.ErrorMessage = "No se puede eliminar la induccion porque tiene registros asociados en otras tablas.";
                objResultado.ErrorSource += $"[{e.Source}]";
            }
            finally
            {
