...
namespace SGUEES.Repositories
{
    public class SC_IMPACTO_ECONOMICORepository : BaseRepository<SC_IMPACTO_ECONOMICOTable>, ISC_IMPACTO_ECONOMICORepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[SC_IMPACTO_ECONOMICORepository]";
                    return objResultado;
                }
        ...
        private async Task<List<SC_IMPACTO_ECONOMICOView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
        {
            var dbWhere = xWhere
            ...
            var estado = xWhere
                .Where(x => x.ParameterName == "ESTADO_IMPACTO_ECONOMICO")
            ...
            {
                response = response
                    .Where(x => (x.ESTADO_IMPACTO_ECONOMICO ?? false) == estado.Value)
                    .ToList();
            }
        ...
        private static List<SC_IMPACTO_ECONOMICOView> ApplyColumnFilters(
            List<SC_IMPACTO_ECONOMICOView> response,
        ...
        private static List<SC_IMPACTO_ECONOMICOView> ApplySort(List<SC_IMPACTO_ECONOMICOView> response, List<CParameter> xWhere)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortField) || !IsAllowedDistinctField(sortField))
            {
                return response
                    ...
                    .ThenBy(x => x.CORR_IMPACTO_ECONOMICO)
                    .ToList();
            }

            var desc = sortDescValue ?? false;
            IEnumerable<SC_IMPACTO_ECONOMICOView> ordered = sortField switch
            {
                "CORR_IMPACTO_ECONOMICO" => desc
                    ? response.OrderByDescending(x => x.CORR_IMPACTO_ECONOMICO)
                    : response.OrderBy(x => x.CORR_IMPACTO_ECONOMICO),
                "DESCRIPCION" => desc
                    ? response.OrderByDescending(x => x.DESCRIPCION)
                    : response.OrderBy(x => x.DESCRIPCION),
                "ESTADO_IMPACTO_ECONOMICO" => desc
                    ? response.OrderByDescending(x => x.ESTADO_IMPACTO_ECONOMICO ?? false)
                    : response.OrderBy(x => x.ESTADO_IMPACTO_ECONOMICO ?? false),
                "USUARIO_CREA" => desc
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_IMPACTO_ECONOMICO),
            };
        ...
        private static bool MatchesExactColumnValue(SC_IMPACTO_ECONOMICOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_IMPACTO_ECONOMICO", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(SC_IMPACTO_ECONOMICOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_IMPACTO_ECONOMICO", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(SC_IMPACTO_ECONOMICOView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
                return !row.ESTADO_IMPACTO_ECONOMICO.HasValue;
            }

            if (!row.ESTADO_IMPACTO_ECONOMICO.HasValue)
            {
                return false;
            }
            ...
            {
                return row.ESTADO_IMPACTO_ECONOMICO.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_IMPACTO_ECONOMICO", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static List<object> CollectDistinctValuesInRowOrder(
            List<SC_IMPACTO_ECONOMICOView> rows,
        ...
        private static object GetDistinctValue(SC_IMPACTO_ECONOMICOView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_IMPACTO_ECONOMICO":
                    return row.CORR_IMPACTO_ECONOMICO;
                case "DESCRIPCION":
                    return row.DESCRIPCION;
                case "ESTADO_IMPACTO_ECONOMICO":
                    return row.ESTADO_IMPACTO_ECONOMICO.HasValue
                        ? row.ESTADO_IMPACTO_ECONOMICO.Value
                        : null;
                case "USUARIO_CREA":
        ...
        private static string GetColumnValue(SC_IMPACTO_ECONOMICOView row, string columnName)
        {
            switch (columnName)
            {
                ...
                case "ESTADO_IMPACTO_ECONOMICO":
                    ...
                    return row.ESTADO_IMPACTO_ECONOMICO.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
