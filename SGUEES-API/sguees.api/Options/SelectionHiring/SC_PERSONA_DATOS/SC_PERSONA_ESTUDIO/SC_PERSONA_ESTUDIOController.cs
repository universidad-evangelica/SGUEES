...
namespace sguees.Controllers
{
    ...
    [Route("[controller]")]
    [ApiController]
    public class SC_PERSONA_ESTUDIOController : ControllerBase
    {
        private readonly ISC_PERSONA_ESTUDIOService _service;
        public SC_PERSONA_ESTUDIOController(ISC_PERSONA_ESTUDIOService service) { _service = service ?? throw new ArgumentNullException(nameof(service)); }
        [HttpGet("GetAll")]
        [Authorize(Policy = "/sc-persona-datos|R")]
        public async Task<CResult> GetAll([FromQuery] SC_PERSONA_ESTUDIOParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAllAsync(Data); }
        [HttpGet("GetAll_SC_SOLICITUD_EMPLEO")]
        [Authorize(Policy = "/sc-solicitud-empleo,/sc-requisicion-personal|R")]
        public async Task<CResult> GetAll_SC_SOLICITUD_EMPLEO([FromQuery] SC_PERSONA_ESTUDIOParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAllAsync(Data); }
        [HttpGet("Get")]
        [Authorize(Policy = "/sc-persona-datos|R")]
        public async Task<CResult> Get([FromQuery] SC_PERSONA_ESTUDIOParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAsync(Data); }
        [HttpPost]
        [Authorize(Policy = "/sc-persona-datos|C")]
        public async Task<IActionResult> Post([FromBody] SC_PERSONA_ESTUDIOTable Data)
        {
        ...
        public async Task<IActionResult> Put([FromBody] SC_PERSONA_ESTUDIOTable Data, [FromQuery] int CORR_PERSONA_DATOS = 0, [FromQuery] int CORR_ESTUDIO = 0)
        {
            Data.CORR_EMPRESA = Empresa(); if (CORR_PERSONA_DATOS > 0) Data.CORR_PERSONA_DATOS = CORR_PERSONA_DATOS; if (CORR_ESTUDIO > 0) Data.CORR_ESTUDIO = CORR_ESTUDIO;
        ...
        public async Task<IActionResult> Delete([FromQuery] SC_PERSONA_ESTUDIOTable Data)
        {
