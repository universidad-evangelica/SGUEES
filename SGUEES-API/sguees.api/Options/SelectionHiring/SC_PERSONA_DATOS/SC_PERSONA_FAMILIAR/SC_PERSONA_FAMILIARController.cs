...
namespace sguees.Controllers
{
    ...
    [Route("[controller]")]
    [ApiController]
    public class SC_PERSONA_FAMILIARController : ControllerBase
    {
        private readonly ISC_PERSONA_FAMILIARService _service;
        public SC_PERSONA_FAMILIARController(ISC_PERSONA_FAMILIARService service) { _service = service ?? throw new ArgumentNullException(nameof(service)); }
        [HttpGet("GetAll")]
        [Authorize(Policy = "/sc-persona-datos|R")]
        public async Task<CResult> GetAll([FromQuery] SC_PERSONA_FAMILIARParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAllAsync(Data); }
        [HttpGet("GetAll_SC_SOLICITUD_EMPLEO")]
        [Authorize(Policy = "/sc-solicitud-empleo,/sc-requisicion-personal|R")]
        public async Task<CResult> GetAll_SC_SOLICITUD_EMPLEO([FromQuery] SC_PERSONA_FAMILIARParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAllAsync(Data); }
        [HttpGet("Get")]
        [Authorize(Policy = "/sc-persona-datos|R")]
        public async Task<CResult> Get([FromQuery] SC_PERSONA_FAMILIARParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAsync(Data); }
        [HttpPost]
        [Authorize(Policy = "/sc-persona-datos|C")]
        public async Task<IActionResult> Post([FromBody] SC_PERSONA_FAMILIARTable Data)
        {
        ...
        public async Task<IActionResult> Put([FromBody] SC_PERSONA_FAMILIARTable Data, [FromQuery] int CORR_PERSONA_DATOS = 0, [FromQuery] int CORR_FAMILIAR = 0)
        {
            Data.CORR_EMPRESA = Empresa(); if (CORR_PERSONA_DATOS > 0) Data.CORR_PERSONA_DATOS = CORR_PERSONA_DATOS; if (CORR_FAMILIAR > 0) Data.CORR_FAMILIAR = CORR_FAMILIAR;
        ...
        public async Task<IActionResult> Delete([FromQuery] SC_PERSONA_FAMILIARTable Data)
        {
