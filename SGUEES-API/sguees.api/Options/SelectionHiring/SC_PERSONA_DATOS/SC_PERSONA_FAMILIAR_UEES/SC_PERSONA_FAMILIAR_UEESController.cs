...
namespace sguees.Controllers
{
    ...
    [Route("[controller]")]
    [ApiController]
    public class SC_PERSONA_FAMILIAR_UEESController : ControllerBase
    {
        private readonly ISC_PERSONA_FAMILIAR_UEESService _service;
        public SC_PERSONA_FAMILIAR_UEESController(ISC_PERSONA_FAMILIAR_UEESService service) { _service = service ?? throw new ArgumentNullException(nameof(service)); }
        [HttpGet("GetAll")]
        [Authorize(Policy = "/sc-persona-datos|R")]
        public async Task<CResult> GetAll([FromQuery] SC_PERSONA_FAMILIAR_UEESParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAllAsync(Data); }
        [HttpGet("GetAll_SC_SOLICITUD_EMPLEO")]
        [Authorize(Policy = "/sc-solicitud-empleo|R")]
        public async Task<CResult> GetAll_SC_SOLICITUD_EMPLEO([FromQuery] SC_PERSONA_FAMILIAR_UEESParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAllAsync(Data); }
        [HttpGet("Get")]
        [Authorize(Policy = "/sc-persona-datos|R")]
        public async Task<CResult> Get([FromQuery] SC_PERSONA_FAMILIAR_UEESParam Data) { Data.CORR_EMPRESA = Empresa(); return await _service.GetAsync(Data); }
        [HttpPost]
        [Authorize(Policy = "/sc-persona-datos|C")]
        public async Task<IActionResult> Post([FromBody] SC_PERSONA_FAMILIAR_UEESTable Data)
        {
        ...
        public async Task<IActionResult> Put([FromBody] SC_PERSONA_FAMILIAR_UEESTable Data, [FromQuery] int CORR_PERSONA_DATOS = 0, [FromQuery] int CORR_FAMILIAR_UEES = 0)
        {
            Data.CORR_EMPRESA = Empresa(); if (CORR_PERSONA_DATOS > 0) Data.CORR_PERSONA_DATOS = CORR_PERSONA_DATOS; if (CORR_FAMILIAR_UEES > 0) Data.CORR_FAMILIAR_UEES = CORR_FAMILIAR_UEES;
        ...
        public async Task<IActionResult> Delete([FromQuery] SC_PERSONA_FAMILIAR_UEESTable Data)
        {
