...
namespace SGUEES.Repositories
{
    public class SC_REQUERIMIENTO_ORGANIZACIONALRepository : BaseRepository<SC_REQUERIMIENTO_ORGANIZACIONALTable>, ISC_REQUERIMIENTO_ORGANIZACIONALRepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[SC_REQUERIMIENTO_ORGANIZACIONALRepository]";
                    return objResultado;
                }
        ...
        private async Task<List<SC_REQUERIMIENTO_ORGANIZACIONALView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
        {
            var dbWhere = xWhere
            ...
            var estado = xWhere
                .Where(x => x.ParameterName == "ESTADO_REQUERIMIENTO_ORGANIZACIONAL")
            ...
            {
                response = response
                    .Where(x => (x.ESTADO_REQUERIMIENTO_ORGANIZACIONAL ?? false) == estado.Value)
                    .ToList();
            }
        ...
        private static List<SC_REQUERIMIENTO_ORGANIZACIONALView> ApplyColumnFilters(
            List<SC_REQUERIMIENTO_ORGANIZACIONALView> response,
        ...
        private static bool MatchesAnyOfFilters(
            SC_REQUERIMIENTO_ORGANIZACIONALView row,
            Dictionary<string, List<string>> anyOfFilters)
        {
        ...
        private static List<SC_REQUERIMIENTO_ORGANIZACIONALView> ApplySort(List<SC_REQUERIMIENTO_ORGANIZACIONALView> response, List<CParameter> xWhere)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortField) || !IsAllowedDistinctField(sortField))
            {
                return response
                    ...
                    .ThenBy(x => x.CORR_REQUERIMIENTO_ORGANIZACIONAL)
                    .ToList();
            }

            var desc = sortDescValue ?? false;
            IEnumerable<SC_REQUERIMIENTO_ORGANIZACIONALView> ordered = sortField switch
            {
                "CORR_REQUERIMIENTO_ORGANIZACIONAL" => desc
                    ? response.OrderByDescending(x => x.CORR_REQUERIMIENTO_ORGANIZACIONAL)
                    : response.OrderBy(x => x.CORR_REQUERIMIENTO_ORGANIZACIONAL),
                "DESCRIPCION" => desc
                    ? response.OrderByDescending(x => x.DESCRIPCION)
                    : response.OrderBy(x => x.DESCRIPCION),
                "ESTADO_REQUERIMIENTO_ORGANIZACIONAL" => desc
                    ? response.OrderByDescending(x => x.ESTADO_REQUERIMIENTO_ORGANIZACIONAL ?? false)
                    : response.OrderBy(x => x.ESTADO_REQUERIMIENTO_ORGANIZACIONAL ?? false),
                "USUARIO_CREA" => desc
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_REQUERIMIENTO_ORGANIZACIONAL),
            };
        ...
        private static bool IsAllowedDistinctField(string field)
        {
            return field switch
            {
                "CORR_REQUERIMIENTO_ORGANIZACIONAL" => true,
                "DESCRIPCION" => true,
        ...
        private static bool MatchesExactColumnValue(SC_REQUERIMIENTO_ORGANIZACIONALView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_REQUERIMIENTO_ORGANIZACIONAL", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(SC_REQUERIMIENTO_ORGANIZACIONALView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_REQUERIMIENTO_ORGANIZACIONAL", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(SC_REQUERIMIENTO_ORGANIZACIONALView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
                return !row.ESTADO_REQUERIMIENTO_ORGANIZACIONAL.HasValue;
            }

            if (!row.ESTADO_REQUERIMIENTO_ORGANIZACIONAL.HasValue)
            {
                return false;
            }
            ...
            {
                return row.ESTADO_REQUERIMIENTO_ORGANIZACIONAL.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_REQUERIMIENTO_ORGANIZACIONAL", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static List<object> CollectDistinctValuesInRowOrder(
            List<SC_REQUERIMIENTO_ORGANIZACIONALView> rows,
        ...
        private static object GetDistinctValue(SC_REQUERIMIENTO_ORGANIZACIONALView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_REQUERIMIENTO_ORGANIZACIONAL":
                    return row.CORR_REQUERIMIENTO_ORGANIZACIONAL;
                case "DESCRIPCION":
                    return row.DESCRIPCION;
                case "ESTADO_REQUERIMIENTO_ORGANIZACIONAL":
                    return row.ESTADO_REQUERIMIENTO_ORGANIZACIONAL.HasValue
                        ? row.ESTADO_REQUERIMIENTO_ORGANIZACIONAL.Value
                        : null;
                case "USUARIO_CREA":
        ...
        private static string GetColumnValue(SC_REQUERIMIENTO_ORGANIZACIONALView row, string columnName)
        {
            switch (columnName)
            {
                ...
                case "ESTADO_REQUERIMIENTO_ORGANIZACIONAL":
                    ...
                    return row.ESTADO_REQUERIMIENTO_ORGANIZACIONAL.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
