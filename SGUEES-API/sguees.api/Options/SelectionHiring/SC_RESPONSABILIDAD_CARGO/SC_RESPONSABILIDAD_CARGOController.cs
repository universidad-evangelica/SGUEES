...
namespace SGUEES.Controllers
{
    ...
    public class SC_RESPONSABILIDAD_CARGOController : ControllerBase
    {
        ...
        [HttpPut("Activar")]
        [Authorize(Policy = "/sc-responsabilidad-cargo|U")]
        public async Task<IActionResult> Activar(SC_RESPONSABILIDAD_CARGOTable Data)
        {
            if (!ValidateEmpresaSesion(out var resultadoEmpresa))
                return BadRequest(resultadoEmpresa);
        ...
        [HttpPut("Desactivar")]
        [Authorize(Policy = "/sc-responsabilidad-cargo|U")]
        public async Task<IActionResult> Desactivar(SC_RESPONSABILIDAD_CARGOTable Data)
        {
            if (!ValidateEmpresaSesion(out var resultadoEmpresa))
                return BadRequest(resultadoEmpresa);
        ...
        private bool ValidateEmpresaSesion(out CResult resultado)
        {
            ...
            resultado = new CResult
            {
                ...
                ErrorSource = "[SC_RESPONSABILIDAD_CARGOController]",
                RowsAffected = 0
            };
