...
namespace SGUEES.Repositories
{
    public class SC_RESPONSABILIDAD_CARGORepository : BaseRepository<SC_RESPONSABILIDAD_CARGOTable>, ISC_RESPONSABILIDAD_CARGORepository
    {
        ...
        public async Task<CResult> GetDistinctValuesAsync(List<CParameter> xWhere)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(distinctField) || !IsAllowedDistinctField(distinctField))
                {
                    ...
                    objResultado.ErrorSource = "[SC_RESPONSABILIDAD_CARGORepository]";
                    return objResultado;
                }
        ...
        private async Task<List<SC_RESPONSABILIDAD_CARGOView>> FilterQueryAsync(List<CParameter> xWhere, string skipColumnFilter = null)
        {
            var dbWhere = xWhere
            ...
            var estado = xWhere
                .Where(x => x.ParameterName == "ESTADO_RESPONSABILIDAD")
            ...
            {
                response = response
                    .Where(x => (x.ESTADO_RESPONSABILIDAD ?? false) == estado.Value)
                    .ToList();
            }
        ...
        private static List<SC_RESPONSABILIDAD_CARGOView> ApplyColumnFilters(
            List<SC_RESPONSABILIDAD_CARGOView> response,
        ...
        private static bool MatchesAnyOfFilters(
            SC_RESPONSABILIDAD_CARGOView row,
            Dictionary<string, List<string>> anyOfFilters)
        {
        ...
        private static List<SC_RESPONSABILIDAD_CARGOView> ApplySort(List<SC_RESPONSABILIDAD_CARGOView> response, List<CParameter> xWhere)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortField) || !IsAllowedDistinctField(sortField))
            {
                return response
                    ...
                    .ThenBy(x => x.CORR_RESPONSABILIDAD)
                    .ToList();
            }

            var desc = sortDescValue ?? false;
            IEnumerable<SC_RESPONSABILIDAD_CARGOView> ordered = sortField switch
            {
                "CORR_RESPONSABILIDAD" => desc
                    ? response.OrderByDescending(x => x.CORR_RESPONSABILIDAD)
                    : response.OrderBy(x => x.CORR_RESPONSABILIDAD),
                "NOMBRE_RESPONSABILIDAD" => desc
                    ? response.OrderByDescending(x => x.NOMBRE_RESPONSABILIDAD)
                    : response.OrderBy(x => x.NOMBRE_RESPONSABILIDAD),
                "ESTADO_RESPONSABILIDAD" => desc
                    ? response.OrderByDescending(x => x.ESTADO_RESPONSABILIDAD ?? false)
                    : response.OrderBy(x => x.ESTADO_RESPONSABILIDAD ?? false),
                "USUARIO_CREA" => desc
                ...
                _ => response.OrderBy(x => x.CORR_EMPRESA).ThenBy(x => x.CORR_RESPONSABILIDAD),
            };
        ...
        private static bool IsAllowedDistinctField(string field)
        {
            return field switch
            {
                "CORR_RESPONSABILIDAD" => true,
                "NOMBRE_RESPONSABILIDAD" => true,
        ...
        private static bool MatchesExactColumnValue(SC_RESPONSABILIDAD_CARGOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_RESPONSABILIDAD", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static bool MatchesAnyOfColumnValue(SC_RESPONSABILIDAD_CARGOView row, string columnName, string filterValue)
        {
            if (string.Equals(columnName, "ESTADO_RESPONSABILIDAD", StringComparison.OrdinalIgnoreCase))
            {
                return MatchesEstadoColumnValue(row, filterValue);
        ...
        private static bool MatchesEstadoColumnValue(SC_RESPONSABILIDAD_CARGOView row, string filterValue)
        {
            if (string.Equals(filterValue, "__BLANK__", StringComparison.OrdinalIgnoreCase))
            {
                return !row.ESTADO_RESPONSABILIDAD.HasValue;
            }

            if (!row.ESTADO_RESPONSABILIDAD.HasValue)
            {
                return false;
            }
            ...
            {
                return row.ESTADO_RESPONSABILIDAD.Value;
            }
        ...
        {
            return string.Equals(columnName, "CORR_RESPONSABILIDAD", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static List<object> CollectDistinctValuesInRowOrder(
            List<SC_RESPONSABILIDAD_CARGOView> rows,
        ...
        private static object GetDistinctValue(SC_RESPONSABILIDAD_CARGOView row, string columnName)
        {
            switch (columnName)
            {
                case "CORR_RESPONSABILIDAD":
                    return row.CORR_RESPONSABILIDAD;
                case "NOMBRE_RESPONSABILIDAD":
                    return row.NOMBRE_RESPONSABILIDAD;
                case "ESTADO_RESPONSABILIDAD":
                    return row.ESTADO_RESPONSABILIDAD.HasValue
                        ? row.ESTADO_RESPONSABILIDAD.Value
                        : null;
                case "USUARIO_CREA":
        ...
        private static string GetColumnValue(SC_RESPONSABILIDAD_CARGOView row, string columnName)
        {
            switch (columnName)
            {
                ...
                case "ESTADO_RESPONSABILIDAD":
                    ...
                    return row.ESTADO_RESPONSABILIDAD.Value ? "Activo" : "Inactivo";
                case "USUARIO_CREA":
                    return row.USUARIO_CREA;
                case "FECHA_CREA":
                    return row.FECHA_CREA?.ToString("dd/MM/yyyy HH:mm");
