...
{
	public class CON_DEPARTAMENTORepository : BaseRepository<CON_DEPARTAMENTOTable>, ICON_DEPARTAMENTORepository
	{
		private const string _TableName = "CON_DEPARTAMENTO";

		public CON_DEPARTAMENTORepository(IConfiguration config) :
		...
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<CON_DEPARTAMENTOView>().FromDataReader(reader).ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader("V_" + _TableName, xWhere);
				var response = new List<CON_DEPARTAMENTOView>().FromDataReader(reader).FirstOrDefault();
				reader.Close(); reader = null;
		...
		public async Task<CResult> CreateAsync(CON_DEPARTAMENTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() {ParameterName="NOMBRE_DEPARTAMENTO",Value=Data.NOMBRE_DEPARTAMENTO,DbType=System.Data.DbType.String},
					new CParameter() {ParameterName="CODIGO_DEPARTAMENTO",Value=Data.CODIGO_DEPARTAMENTO,DbType=System.Data.DbType.String},
				};
				var pWhere = new List<CParameter>
				{
					new CParameter() {ParameterName="CORR_EMPRESA",Value=Data.CORR_EMPRESA,DbType=System.Data.DbType.Int32},
				};
				var reader = await objData.Insert(_TableName, p, "CORR_DEPARTAMENTO", pWhere);
				var response = new List<CON_DEPARTAMENTOView>().FromDataReader(reader).FirstOrDefault();
				objResultado.Data = response; objResultado.Result = true; objResultado.RowsAffected = 1;
				objResultado.CodeHelper = 0; objResultado.ErrorCode = 0; objResultado.ErrorMessage = ""; objResultado.ErrorSource = "";
			}
		...
		public async Task<CResult> UpdateAsync(CON_DEPARTAMENTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			try
			{
		...
		public async Task<CResult> DeleteAsync(CON_DEPARTAMENTOTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			try
			{
