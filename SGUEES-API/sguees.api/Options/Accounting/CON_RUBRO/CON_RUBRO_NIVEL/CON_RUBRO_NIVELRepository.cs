...
{
	public class CON_RUBRO_NIVELRepository : BaseRepository<CON_RUBRO_NIVELTable>, ICON_RUBRO_NIVELRepository
	{
		private const string _TableName = "CON_RUBRO_NIVEL";

		public CON_RUBRO_NIVELRepository(IConfiguration config) :
		...
		public async Task<CResult> GetAllAsync(List<CParameter> xWhere)
		{
			...
			try
			{
				...
				var response = new List<CON_RUBRO_NIVELView>().FromDataReader(reader).ToList();
		...
		public async Task<CResult> GetAsync(List<CParameter> xWhere)
		{
			...
			{
				var reader = await objData.GetDataReader("V_" + _TableName, xWhere);
				var response = new List<CON_RUBRO_NIVELView>().FromDataReader(reader).FirstOrDefault();
				reader.Close(); reader = null;
		...
		public async Task<CResult> CreateAsync(CON_RUBRO_NIVELTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			try
			{
				var p = new List<CParameter>
				{
					...
					new CParameter() {ParameterName="NUMERO_CARACTERES",Value=Data.NUMERO_CARACTERES,DbType=System.Data.DbType.Int16},
				};
		...
		public async Task<CResult> UpdateAsync(CON_RUBRO_NIVELTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			try
			{
		...
		public async Task<CResult> DeleteAsync(CON_RUBRO_NIVELTable Data, string vLOGIN_SISTEMA, string vESTACION)
		{
			CResult objResultado = new();
			try
			{
