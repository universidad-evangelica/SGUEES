...
namespace sguees.Controllers
{
	...
	[Route("[controller]")]
	[ApiController]
	public class CON_RUBROController : ControllerBase
	{
		private readonly ICON_RUBROService _service;

		public CON_RUBROController(ICON_RUBROService service)
		{
		...
		public async Task<CResult> GetAll([FromQuery] CON_RUBROParam Data)
		{
		...
		[HttpGet("Get")]
		[Authorize(Policy = "/con-rubro|R")]
		public async Task<CResult> Get([FromQuery] CON_RUBROParam Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
		...
		public async Task<IActionResult> Post(CON_RUBROTable Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
		...
		public async Task<IActionResult> Put(CON_RUBROTable Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
		...
		public async Task<IActionResult> Delete([FromQuery] CON_RUBROTable Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
