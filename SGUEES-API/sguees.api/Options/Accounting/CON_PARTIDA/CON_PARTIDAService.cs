...

{
...

	{
		...
		{
			var rows = await LoadPartidaImprRowsAsync(xWhere);
			var stream = await _repoRpt.GetConPartidaImprAsync(
				rows,
			...
			if (stream == null)
			{
				throw new System.InvalidOperationException(
					"No se pudo generar el PDF en el servicio de reportes. Verifique que SGUEES-RPT esté activo.");
			}

			return stream;
		}

		public async Task<Stream> GetPDFDxAsync(CON_PARTIDAParam xWhere, string loginSistema)
		{
			var rows = await LoadPartidaImprRowsAsync(xWhere);
			var stream = await _repoRpt.GetConPartidaImprDxAsync(
				rows,
				_repoUser.GenerateRptToken(loginSistema));
			...
			{
				throw new System.InvalidOperationException(
					"No se pudo generar el PDF DevExpress en SGUEES-RPT. Verifique que el sitio esté activo.");
			}

			return stream;
		}

		private async Task<List<CON_PARTIDA_IMPRView>> LoadPartidaImprRowsAsync(CON_PARTIDAParam xWhere)
		{
			// e-Admin Imprimir(): filtra por ANIO/MES/CLASE/PARTIDA sin rango de fechas.
			var useDateFilter = xWhere.CORR_PARTIDA <= 0;
			var p = new List<CParameter>
			{
				new CParameter() { ParameterName = "@CORR_EMPRESA", Value = xWhere.CORR_EMPRESA, DbType = System.Data.DbType.Int32 },
				new CParameter() { ParameterName = "@FECHA_INICIAL", Value = useDateFilter ? xWhere.FECHA_INICIAL : null, DbType = System.Data.DbType.Date },
				new CParameter() { ParameterName = "@FECHA_FINAL", Value = useDateFilter ? xWhere.FECHA_FINAL : null, DbType = System.Data.DbType.Date },
				new CParameter() { ParameterName = "@ANIO_PERIODO", Value = xWhere.ANIO_PERIODO, DbType = System.Data.DbType.Int32 },
				new CParameter() { ParameterName = "@MES_PERIODO", Value = xWhere.MES_PERIODO, DbType = System.Data.DbType.Int32 },
				new CParameter() { ParameterName = "@CORR_CLASE_PARTIDA", Value = xWhere.CORR_CLASE_PARTIDA, DbType = System.Data.DbType.Int32 },
				new CParameter() { ParameterName = "@CORR_PARTIDA", Value = xWhere.CORR_PARTIDA, DbType = System.Data.DbType.Int32 },
			...
			{
				throw new System.InvalidOperationException(
					string.IsNullOrWhiteSpace(vRespuestaPartidaData.ErrorMessage)
						? "No hay datos para imprimir la partida."
						: vRespuestaPartidaData.ErrorMessage);
			}

			return (List<CON_PARTIDA_IMPRView>)vRespuestaPartidaData.Data;
		}
