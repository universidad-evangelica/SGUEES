...
namespace sguees.Controllers
{
	...
	[Route("[controller]")]
	[ApiController]
	public class CON_CLASE_PARTIDAController : ControllerBase
	{
		private readonly ICON_CLASE_PARTIDAService _service;
		public CON_CLASE_PARTIDAController(ICON_CLASE_PARTIDAService service) { _service = service ?? throw new ArgumentNullException(nameof(_service)); }

		[HttpGet("GetAll")]
		[Authorize(Policy = "/con-clase-partida|R")]
		public async Task<CResult> GetAll([FromQuery] CON_CLASE_PARTIDAParam Data)
		{
		...
		[HttpGet("Get")]
		[Authorize(Policy = "/con-clase-partida|R")]
		public async Task<CResult> Get([FromQuery] CON_CLASE_PARTIDAParam Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
		...
		[HttpPost]
		[Authorize(Policy = "/con-clase-partida|C")]
		public async Task<IActionResult> Post(CON_CLASE_PARTIDATable Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
		...
		[HttpPut]
		[Authorize(Policy = "/con-clase-partida|U")]
		public async Task<IActionResult> Put(CON_CLASE_PARTIDATable Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
		...
		[HttpDelete]
		[Authorize(Policy = "/con-clase-partida|D")]
		public async Task<IActionResult> Delete([FromQuery] CON_CLASE_PARTIDATable Data)
		{
			Data.CORR_EMPRESA = int.Parse(User.Claims.ToList().SingleOrDefault(e => e.Type == "CORR_EMPRESA").Value);
