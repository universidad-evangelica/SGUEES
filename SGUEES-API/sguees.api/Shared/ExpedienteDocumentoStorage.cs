...
namespace sguees.api.Shared
{
	public class ExpedienteDocumentoStorage
	{
		...
		private readonly ExpedienteDocumentoOptions _options;

		public ExpedienteDocumentoStorage(IWebHostEnvironment environment, ExpedienteDocumentoOptions options)
		{
			_rootPath = Path.Combine(environment.ContentRootPath, "uploads", "sc-expediente-documento");
			_options = options ?? new ExpedienteDocumentoOptions();
		}

		public int MaxBytes => _options.MaxBytes > 0 ? _options.MaxBytes : 10 * 1024 * 1024;

		public string BuildRelativeUrl(int corrEmpresa, int corrExpediente, int corrDocumento, string extension)
		{
			var ext = NormalizeExtension(extension);
			var fileName = $"{corrEmpresa}_{corrExpediente}_{corrDocumento}{ext}";
			return $"/uploads/sc-expediente-documento/{fileName}";
		}
		...
		public bool TryResolvePhysicalPath(string relativeUrl, out string physicalPath)
		{
			...
			const string prefix = "uploads/sc-expediente-documento/";
			...
			if (!Regex.IsMatch(fileName, @"^\d+_\d+_\d+\.[a-z0-9]+$", RegexOptions.IgnoreCase))
			{
				return false;
			}
			...
			IFormFile file)
		{
			...
			if (corrEmpresa <= 0 || corrExpediente <= 0 || corrDocumento <= 0)
			{
				return (false, "Identificador de documento inválido.", null, null);
			}

			Directory.CreateDirectory(_rootPath);

			var extension = NormalizeExtension(Path.GetExtension(file.FileName));
			var storedFileName = $"{corrEmpresa}_{corrExpediente}_{corrDocumento}{extension}";
			...
			return (true, null, BuildRelativeUrl(corrEmpresa, corrExpediente, corrDocumento, extension), Path.GetFileName(file.FileName));
		}
