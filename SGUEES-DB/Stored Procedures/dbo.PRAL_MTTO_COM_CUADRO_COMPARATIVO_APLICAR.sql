...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[PRAL_MTTO_COM_CUADRO_COMPARATIVO_APLICAR]
(
	@CORR_EMPRESA INT,
	@ANIO_PERIODO INT,
...
AS
	DECLARE @CORR_PROVEEDOR INT,@CODIGO_PROVEEDOR VARCHAR(25),@IDPREORDEN BIGINT,@ANIO_PERIODO_SOLI_COTI INT,@CORR_SOLI_COTIZACION INT

	DECLARE @PROVEEDORES TABLE(CORR_PROVEEDOR INT,CODIGO_PROVEEDOR VARCHAR(25),ANIO_PERIODO_SOLI_COTI INT,CORR_SOLI_COTIZACION INT)
	...
	@CODIGO_OPCION VARCHAR(150)='COM_CUADRO_COMPARATIVO',@CLASE_BITACORA VARCHAR(3)='AP'
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS(SELECT 1 FROM dbo.COM_CUADRO_COMPARATIVO A 
				  WHERE A.CORR_EMPRESA=@CORR_EMPRESA
				  AND A.ANIO_PERIODO=@ANIO_PERIODO
				  AND A.ANIO_PERIODO=@CORR_CUADRO_COMPARATIVO
				  AND A.ESTADO_CUADRO_COMPARATIVO<>'DI'
				  )
		BEGIN
			SELECT @SYS_NUMERO_ERROR=30002,@SYS_MENSAJE_ERROR='el cuadro comparativo debe estar en digitado. se permite aplicar cuadro comparativo.',@SYS_FILAS_AFECTADAS=0
			GOTO FINA
		END

		IF NOT EXISTS(SELECT 1 FROM dbo.COM_CUADRO_COMPARATIVO_DETA A 
				  WHERE A.CORR_EMPRESA=@CORR_EMPRESA
				  AND A.ANIO_PERIODO=@ANIO_PERIODO
				  AND A.CORR_CUADRO_COMPARATIVO=@CORR_CUADRO_COMPARATIVO
				  )
		BEGIN
			SELECT @SYS_NUMERO_ERROR=30002,@SYS_MENSAJE_ERROR='No existen item en el detalle del cuadro comparativo. se permite aplicar cuadro comparativo.',@SYS_FILAS_AFECTADAS=0
			GOTO FINA
		END
		...
		BEGIN
			SELECT @SYS_NUMERO_ERROR=30002,@SYS_MENSAJE_ERROR='No existen item seleccionados en el detalle del cuadro comparativo. se permite aplicar cuadro comparativo.',@SYS_FILAS_AFECTADAS=0
			GOTO FINA
		END
