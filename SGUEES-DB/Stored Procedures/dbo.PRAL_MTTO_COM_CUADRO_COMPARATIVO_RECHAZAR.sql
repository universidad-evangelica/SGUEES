...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[PRAL_MTTO_COM_CUADRO_COMPARATIVO_RECHAZAR]
(
	@CORR_EMPRESA int,
	@ANIO_PERIODO INT,
...
AS
	...
	@CODIGO_OPCION VARCHAR(150)='COM_CUADRO_COMPARATIVO',@CLASE_BITACORA VARCHAR(3)='RE'
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			SELECT @SYS_NUMERO_ERROR=30002,@SYS_MENSAJE_ERROR='el cuadro comparativo debe estar en digitado. no se permite rechazar cuadro comparativo.',@SYS_FILAS_AFECTADAS=0
			GOTO FINA
		END
		...
		--INNER JOIN COM_CUADRO_COMPARATIVO_ORDEN_COMPRA B ON A.ORDNUM=B.NUMERO_ORDEN
		--WHERE B.CORR_EMPRESA=@CORR_EMPRESA
		--AND B.ANIO_PERIODO=@ANIO_PERIODO
		--AND B.CORR_CUADRO_COMPARATIVO=@CORR_CUADRO_COMPARATIVO
